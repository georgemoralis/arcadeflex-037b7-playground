...
public class bionicc
{
	...
	static struct_tilemap tx_tilemap, bg_tilemap, fg_tilemap;
	...
	***************************************************************************/
	...
	public static GetTileInfoPtr get_bg_tile_info = new GetTileInfoPtr() { public void handler(int tile_index) 
	{
		...
		int attr = videoram1.read(2*tile_index+1);
		SET_TILE_INFO(1,(videoram1.read(2*tile_index) & 0xff) + ((attr & 0x07) << 8),(attr & 0x18) >> 3);
		tile_info.u32_flags = TILE_FLIPXY((attr & 0xc0) >> 6);
	} };
	...
	public static GetTileInfoPtr get_fg_tile_info = new GetTileInfoPtr() { public void handler(int tile_index) 
	{
		...
		int attr = videoram1.read(2*tile_index+1);
		SET_TILE_INFO(2,(videoram1.read(2*tile_index) & 0xff) + ((attr & 0x07) << 8),(attr & 0x18) >> 3);
		if ((attr & 0xc0) == 0xc0)
		{
			tile_info.u32_priority = 1;
			tile_info.u32_flags = TILE_SPLIT(0);
		}
		else
		{
			tile_info.u32_priority = 0;
			tile_info.u32_flags = TILE_SPLIT((attr & 0x20) >> 5) | TILE_FLIPXY((attr & 0xc0) >> 6);
		}
	} };
	...
	public static GetTileInfoPtr get_tx_tile_info = new GetTileInfoPtr() { public void handler(int tile_index) 
	{
		...
		int attr = videoram1.read(tile_index + 0x400);
		SET_TILE_INFO(0,(videoram1.read(tile_index) & 0xff) + ((attr & 0x00c0) << 2),attr & 0x3f);
	} };
	...
	***************************************************************************/
	...
	public static VhStartPtr bionicc_vh_start = new VhStartPtr() { public int handler() 
	{
		...
		fg_tilemap.u32_transmask[0] = 0xffff; /* split type 0 is completely transparent in front half */
		fg_tilemap.u32_transmask[1] = 0xffc1; /* split type 1 has pens 1-5 opaque in front half */
