...
import static arcadeflex.WIP.v037b7.drivers.system16.sys16_spriteram;
import static arcadeflex.WIP.v037b7.drivers.system16.sys16_textram;
import static arcadeflex.WIP.v037b7.drivers.system16.sys16_tileram;
...
{
/*TODO*///	
/*TODO*///	//#define SYS16_DEBUG
/*TODO*///	//#define SPACEHARRIER_OFFSETS
/*TODO*///	
/*TODO*///	// an attempt at fudging correct gamma for sys16 games, but I'm not sure that it's
/*TODO*///	// really worth using.
/*TODO*///	//#define GAMMA_ADJUST
/*TODO*///	#define TRANSPARENT_SHADOWS
	public static int MAXCOLOURS = 8192;
	
/*TODO*///	#ifdef TRANSPARENT_SHADOWS
	public static int ShadowColorsShift = 8;
...
/*TODO*///	#endif
	...
	public static int sys16_MaxShadowColors_Shift;
        ...
        public static int NUM_SPRITES = 128;

/*TODO*///	extern UINT8 *sys16_textram;
/*TODO*///	extern UINT8 *sys16_spriteram;
/*TODO*///	extern UINT8 *sys16_tileram; /* contains tilemaps for 16 pages */
/*TODO*///	
...
/*TODO*///	int sys16_bg2_scrollx, sys16_bg2_scrolly;
        public static int[] sys16_bg2_page = new int[4];
/*TODO*///	int sys16_fg2_scrollx, sys16_fg2_scrolly;
	public static int[] sys16_fg2_page = new int[4];

/*TODO*///	int sys18_bg2_active;
/*TODO*///	int sys18_fg2_active;
...
/*TODO*///	#endif
/*TODO*///	UBytePtr gr_ver;
/*TODO*///	UBytePtr gr_hor;
/*TODO*///	UBytePtr gr_pal;
/*TODO*///	UBytePtr gr_flip;
/*TODO*///	int gr_palette;
/*TODO*///	int gr_palette_default;
/*TODO*///	unsigned char gr_colorflip[2][4];
/*TODO*///	UBytePtr gr_second_road;

	static struct_tilemap background, foreground, text_layer;
	static struct_tilemap background2, foreground2;
...
/*TODO*///	static void draw_quartet_title_screen( struct osd_bitmap *bitmap,int playfield );
	...
	static GetMemoryOffsetPtr sys16_bg_map = new GetMemoryOffsetPtr() {
            @Override
	...
	static GetMemoryOffsetPtr sys16_text_map = new GetMemoryOffsetPtr() {
            @Override
            public int handler(int col, int row, int num_cols, int num_rows) {
	...
	public static WriteHandlerPtr sys16_paletteram_w = new WriteHandlerPtr() {public void handler(int offset, int data){
		...
		if( oldword!=newword ){
			...
			/*	GBGR BBBB GGGG RRRR */
			/*	5444 3210 3210 3210 */
			int r = ((newword & 0x00f)<<1) & 0xff;
			int g = ((newword & 0x0f0)>>2) & 0xff;
			int b = ((newword & 0xf00)>>7) & 0xff;
			if( sys16_dactype == 0 ){
...
/*TODO*///			palette_change_color( offset/2,
/*TODO*///					(r << 3) | (r >> 2), /* 5 bits red */
/*TODO*///					(g << 2) | (g >> 4), /* 6 bits green */
/*TODO*///					(b << 3) | (b >> 2) /* 5 bits blue */
/*TODO*///				);
/*TODO*///	#else
...
/*TODO*///	#endif
	...
	static GetTileInfoPtr get_bg_tile_info = new GetTileInfoPtr() {
		...
		case 1: // Alien Syndrome
			tile_info.u32_priority = (data&0x8000)!=0?1:0;
			break;
		case 2: // Body Slam / wrestwar
			tile_info.u32_priority = ((data&0xff00) >= sys16_bg_priority_value)?1:0;
			break;
		case 3: // sys18 games
			if ((data & 0x8000) != 0){
				tile_info.u32_priority = 2;
			}
			else {
				tile_info.u32_priority = ((data&0xff00) >= sys16_bg_priority_value)?1:0;
			}
	...
	static GetTileInfoPtr get_fg_tile_info = new GetTileInfoPtr() {
		...
		case 3:
			tile_info.u32_priority = ((data&0xff00) >= sys16_fg_priority_value)?1:0;
			break;
...
/*TODO*///	static void get_bg2_tile_info( int offset ){
/*TODO*///		const UINT16 *source = 64*32*sys16_bg2_page[offset/(64*32)] + (UINT16 *)sys16_tileram;
/*TODO*///		int data = source[offset%(64*32)];
/*TODO*///		int tile_number = (data&0xfff) + 0x1000*((data&0x1000)?sys16_tile_bank1:sys16_tile_bank0);
...
/*TODO*///	}
/*TODO*///	
/*TODO*///	static void get_fg2_tile_info( int offset ){
/*TODO*///		const UINT16 *source = 64*32*sys16_fg2_page[offset/(64*32)] + (UINT16 *)sys16_tileram;
/*TODO*///		int data = source[offset%(64*32)];
/*TODO*///		int tile_number = (data&0xfff) + 0x1000*((data&0x1000)?sys16_tile_bank1:sys16_tile_bank0);
/*TODO*///		if(sys16_textmode==0){
/*TODO*///			SET_TILE_INFO( 0, tile_number, (data>>6)&0x7f );
...
/*TODO*///			SET_TILE_INFO( 0, tile_number, (data>>5)&0x7f );
/*TODO*///		}
/*TODO*///		if((data&0xff00) >= sys16_fg_priority_value) tile_info.priority = 1;
/*TODO*///		else tile_info.priority = 0;
/*TODO*///	}
	...
	static GetTileInfoPtr get_text_tile_info = new GetTileInfoPtr() {
		...
		int tile_number = source.read(offset);
...
/*TODO*///	#ifdef SPACEHARRIER_OFFSETS
/*TODO*///		if (spaceharrier_patternoffsets != 0) free(spaceharrier_patternoffsets);
...
/*TODO*///	#ifdef GAMMA_ADJUST
/*TODO*///			{
/*TODO*///				static float sys16_orig_gamma=0;
/*TODO*///				static float sys16_set_gamma=0;
/*TODO*///				float cur_gamma=osd_get_gamma();
/*TODO*///	
/*TODO*///				if(sys16_orig_gamma == 0)
/*TODO*///				{
/*TODO*///					sys16_orig_gamma = cur_gamma;
/*TODO*///					sys16_set_gamma = cur_gamma - 0.35;
/*TODO*///					if (sys16_set_gamma < 0.5) sys16_set_gamma = 0.5;
/*TODO*///					if (sys16_set_gamma > 2.0) sys16_set_gamma = 2.0;
/*TODO*///					osd_set_gamma(sys16_set_gamma);
/*TODO*///				}
/*TODO*///				else
/*TODO*///				{
/*TODO*///					if(sys16_orig_gamma == cur_gamma)
/*TODO*///					{
/*TODO*///						osd_set_gamma(sys16_set_gamma);
/*TODO*///					}
/*TODO*///				}
...
/*TODO*///		int i;
/*TODO*///	
/*TODO*///		if (!sys16_refreshenable) return;
/*TODO*///	
...
/*TODO*///	{
/*TODO*///		const UINT16 *source;
/*TODO*///		int row,col,r,c;
/*TODO*///		int data;
/*TODO*///	
/*TODO*///		if(!keyboard_pressed_memory(KEYCODE_Y)) return;
/*TODO*///	
/*TODO*///		logerror("Back %2x %2x %2x %2x\n",sys16_bg_page[0],sys16_bg_page[1],sys16_bg_page[2],sys16_bg_page[3]);
/*TODO*///		for(r=0;r<64;r++)
/*TODO*///		{
/*TODO*///			for(c=0;c<128;c++)
/*TODO*///			{
/*TODO*///				source = (const UINT16 *)sys16_tileram;
/*TODO*///				row=r;col=c;
/*TODO*///				if( row<32 ){
/*TODO*///					if( col<64 ){
/*TODO*///						source += 64*32*sys16_bg_page[0];
/*TODO*///					}
/*TODO*///					else {
/*TODO*///						source += 64*32*sys16_bg_page[1];
/*TODO*///					}
/*TODO*///				}
/*TODO*///				else {
/*TODO*///					if( col<64 ){
/*TODO*///						source += 64*32*sys16_bg_page[2];
/*TODO*///					}
/*TODO*///					else {
/*TODO*///						source += 64*32*sys16_bg_page[3];
/*TODO*///					}
/*TODO*///				}
/*TODO*///				row = row%32;
/*TODO*///				col = col%64;
/*TODO*///	
/*TODO*///				data = source[row*64+col];
/*TODO*///				logerror("%4x ",data);
/*TODO*///			}
/*TODO*///			logerror("\n");
/*TODO*///		}
/*TODO*///	
/*TODO*///		logerror("Front %2x %2x %2x %2x\n",sys16_fg_page[0],sys16_fg_page[1],sys16_fg_page[2],sys16_fg_page[3]);
/*TODO*///		for(r=0;r<64;r++)
/*TODO*///		{
/*TODO*///			for(c=0;c<128;c++)
/*TODO*///			{
...
/*TODO*///				if( row<32 ){
/*TODO*///					if( col<64 ){
/*TODO*///						source += 64*32*sys16_fg_page[0];
/*TODO*///					}
/*TODO*///					else {
/*TODO*///						source += 64*32*sys16_fg_page[1];
/*TODO*///					}
/*TODO*///				}
/*TODO*///				else {
/*TODO*///					if( col<64 ){
/*TODO*///						source += 64*32*sys16_fg_page[2];
/*TODO*///					}
/*TODO*///					else {
/*TODO*///						source += 64*32*sys16_fg_page[3];
/*TODO*///					}
/*TODO*///				}
/*TODO*///				row = row%32;
/*TODO*///				col = col%64;
...
/*TODO*///			for(c=0;c<64;c++)
/*TODO*///			{
/*TODO*///				source = (const UINT16 *)sys16_textram;
/*TODO*///				data = source[r*64+c];
/*TODO*///				logerror("%4x ",data);
/*TODO*///			}
/*TODO*///			logerror("\n");
/*TODO*///		}
