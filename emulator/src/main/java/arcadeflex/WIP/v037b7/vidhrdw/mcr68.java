...
public class mcr68
{
	...
	public static int LOW_BYTE(UBytePtr u, int x) {
            return (u.READ_WORD(x) & 0xff);
        }
...
/*TODO*///	#endif
	...
	public static VhConvertColorPromPtr zwackery_convert_color_prom = new VhConvertColorPromPtr() {
		...
		for (code = 0; code < gfx0.total_elements; code++)
		{
			...
			UBytePtr gfxdata0 = new UBytePtr(gfx0.gfxdata, code * gfx0.char_modulo);
			UBytePtr gfxdata2 = new UBytePtr(gfx2.gfxdata, code * gfx2.char_modulo);
	...
	static void zwackery_mark_background()
	{
		...
		for (offs = videoram_size[0] - 2; offs >= 0; offs -= 2)
		{
			...
			for (i = 0; i < 32; i++)
				used_colors.write(coldata.readinc(), PALETTE_COLOR_VISIBLE);
		}
	}
