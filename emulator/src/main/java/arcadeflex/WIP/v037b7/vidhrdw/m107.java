...
public class m107
{
	...
	static struct_tilemap pf4_layer,pf3_layer,pf2_layer,pf1_layer;
	static int[] m107_control = new int[0x20];
	public static UBytePtr m107_spriteram = new UBytePtr();
	public static UBytePtr m107_vram_data = new UBytePtr();
	public static int m107_raster_irq_position,m107_sprite_list;
	...
	public static GetTileInfoPtr get_pf1_tile_info = new GetTileInfoPtr() { public void handler(int tile_index) 
	{
		...
		tile=m107_vram_data.read(tile_index)+(m107_vram_data.read(tile_index+1)<<8);
		if ((m107_vram_data.read(tile_index+3) & 0x10)!=0) tile+=0x10000;
		color=m107_vram_data.read(tile_index+2);
	
		SET_TILE_INFO(0,tile,color&0x7f);
	
		/* Priority 1 = tile appears above sprites */
		tile_info.u32_priority = ((m107_vram_data.read(tile_index+3)&2)>>1);
		tile_info.u32_flags = TILE_FLIPYX((m107_vram_data.read(tile_index+3) & 0xc)>>2);
	} };
	...
	{
		return m107_vram_data.read(offset);
	} };
	...
	public static WriteHandlerPtr m107_vram_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
		...
		m107_vram_data.write(offset, data);
	...
	public static VhStartPtr m107_vh_start = new VhStartPtr() { public int handler() 
	{
		...
		if (pf1_layer==null || pf2_layer==null || pf3_layer==null || pf4_layer==null)
			return 1;
	...
	//	pf3_layer.transmask[1] = 0xff00;
		...
		m107_spriteram = new UBytePtr(0x1000);
	...
	static void mark_sprite_colours()
	{
		...
		for (offs = 0x1000-8;offs >= 0;offs -= 8)
		{
			int sprite,x_multi,multi;
                        ...
                        sprite=m107_spriteram.read(offs+2) | (m107_spriteram.read(offs+3)<<8);
			color=m107_spriteram.read(offs+4)&0x7f;
			x_multi=(m107_spriteram.read(offs+1)>>3)&0x3;
	...
	static void m107_drawsprites(osd_bitmap bitmap, rectangle clip, int pri)
	{
		int offs;
		...
		for (offs = 0x1000-8;offs >= 0;offs -= 8) {
			...
			if (((m107_spriteram.read(offs+4)&0x80)==0x80) && pri==0) continue;
			if (((m107_spriteram.read(offs+4)&0x80)==0x00) && pri==1) continue;
	
			y=m107_spriteram.read(offs+0) | (m107_spriteram.read(offs+1)<<8);
			x=m107_spriteram.read(offs+6) | (m107_spriteram.read(offs+7)<<8);
			x&=0x1ff;
			y&=0x1ff;
	
			if (x==0 || y==0) continue; /* offscreen */
	
                        sprite=(m107_spriteram.read(offs+2) | (m107_spriteram.read(offs+3)<<8))&0x7fff;
	
			x = x - 16;
			y = 512 - 16 - y;
	
			colour=m107_spriteram.read(offs+4)&0x7f;
			fx=m107_spriteram.read(offs+5)&1;
			fy=m107_spriteram.read(offs+5)&2;
			y_multi=(m107_spriteram.read(offs+1)>>3)&0x3;
			x_multi=(m107_spriteram.read(offs+1)>>3)&0x3;
	...
	public static VhUpdatePtr m107_vh_screenrefresh = new VhUpdatePtr() { public void handler(osd_bitmap bitmap,int full_refresh) 
	{
	...
	static void m107_update_scroll_positions()
	{
		int i;
		...
		*/
		...
		if (pf1_rowscroll != 0) {
			...
			for (i=0; i<1024; i+=2)
				tilemap_set_scrollx( pf1_layer,i/2, (m107_vram_data.read(0xe800+i)+(m107_vram_data.read(0xe801+i)<<8)));
		} else {
		...
		if (pf2_rowscroll != 0) {
			...
			for (i=0; i<1024; i+=2)
				tilemap_set_scrollx( pf2_layer,i/2, (m107_vram_data.read(0xf400+i)+(m107_vram_data.read(0xf401+i)<<8)));
		} else {
		...
		if (pf3_rowscroll != 0) {
			...
			for (i=0; i<1024; i+=2)
				tilemap_set_scrollx( pf3_layer,i/2, (m107_vram_data.read(0xf800+i)+(m107_vram_data.read(0xf801+i)<<8)));
		} else {
		...
		if (pf4_rowscroll != 0) {
			...
			for (i=0; i<1024; i+=2)
				tilemap_set_scrollx( pf4_layer,i/2, (m107_vram_data.read(0xfc00+i)+(m107_vram_data.read(0xfc01+i)<<8)));
		} else {
	...
	static void m107_screenrefresh(osd_bitmap bitmap,rectangle clip)
	{
	...
	public static void m107_vh_raster_partial_refresh(osd_bitmap bitmap,int start_line,int end_line)
	{
