...
public class m72
{
	...
	public static UBytePtr m72_videoram1=new UBytePtr(), m72_videoram2=new UBytePtr(), majtitle_rowscrollram=new UBytePtr();
	...
	public static InitMachinePtr poundfor_init_machine = new InitMachinePtr() { public void handler() 
	{
		irq1 = 0x18;
		irq2 = 0x1a;
		m72_init_sound.handler();
	...
	***************************************************************************/
	...
	{
		int attr = m72_videoram2.read(4*tile_index+1);
		SET_TILE_INFO(2,m72_videoram2.read(4*tile_index) + ((attr & 0x3f) << 8),m72_videoram2.read(4*tile_index+2) & 0x0f);
	...
	{
		int attr = m72_videoram1.read(4*tile_index+1);
		SET_TILE_INFO(1,m72_videoram1.read(4*tile_index) + ((attr & 0x3f) << 8),m72_videoram1.read(4*tile_index+2) & 0x0f);
	/* bchopper: (videoram.read(4*tile_index+2)& 0x10) is used, priority? */
		tile_info.u32_flags = TILE_FLIPYX((attr & 0xc0) >> 6);
	
		tile_info.u32_priority = (m72_videoram1.read(4*tile_index+2) & 0x80) >> 7;
	} };
	...
	public static GetTileInfoPtr dbreed_get_bg_tile_info = new GetTileInfoPtr() { public void handler(int tile_index) 
	{
		...
		tile_info.u32_priority = (m72_videoram2.read(4*tile_index+2) & 0x80) >> 7;
	} };
	
	public static GetTileInfoPtr rtype2_get_bg_tile_info = new GetTileInfoPtr() { public void handler(int tile_index) 
	{
		int attr = m72_videoram2.read(4*tile_index+2);
		SET_TILE_INFO(1,m72_videoram2.read(4*tile_index) + (m72_videoram2.read(4*tile_index+1) << 8),attr & 0x0f);
	...
	{
		int attr = m72_videoram1.read(4*tile_index+2);
		SET_TILE_INFO(1,m72_videoram1.read(4*tile_index) + (m72_videoram1.read(4*tile_index+1) << 8),attr & 0x0f);
		tile_info.u32_flags = TILE_FLIPYX((attr & 0x60) >> 5);
	
		tile_info.u32_priority = m72_videoram1.read(4*tile_index+3) & 0x01;
	...
	static void hharry_get_tile_info(int gfxnum,UBytePtr videoram,int tile_index)
	{
		int attr = videoram.read(4*tile_index+1);
	...
	***************************************************************************/
	...
	static void draw_layer(osd_bitmap bitmap,
			struct_tilemap tilemap,int[] scrollx,int[] scrolly,int priority)
	{
		int start,i;
	...
	public static VhEofCallbackPtr m72_eof_callback = new VhEofCallbackPtr() { public void handler() 
	{
		int i;
