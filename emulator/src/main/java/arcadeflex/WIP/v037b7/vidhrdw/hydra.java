...
public class hydra
{
	...
	public static int XCHARS = 42;
	public static int YCHARS = 30;
	
	public static int XDIM = (XCHARS*(8 << HIGH_RES));
	public static int YDIM = (YCHARS*8);
	
	
	public static int DEBUG_VIDEO = 0;
...
/*TODO*///	#endif
		...
		for (x = 1; x < 256; x++)
			for (current = list_head[x]; current!=null; current = current.next)
				mo_render_callback(new UShortPtr(atarigen_spriteram, current.entry * 16), hydra_mo_area, modata);
	...
	static void mo_render_callback(UShortPtr data, rectangle clip, Object param)
	{
		int scale = data.read(4);
		int code = data.read(0) & 0x7fff;
		if (scale > 0x0000 && code < atarigen_rle_count)
		{
			...
			int x = (data.read(2) >> 6)&0xffff;
			int y = (data.read(3) >> 6)&0xffff;
