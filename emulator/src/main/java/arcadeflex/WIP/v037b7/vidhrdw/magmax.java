...
public class magmax
{
	...
	public static int magmax_videoram_size;
	...
	***************************************************************************/
	...
	static void COLOR(char []colortable, int gfxn, int offs, int value){
	...
	public static VhUpdatePtr magmax_vh_screenrefresh = new VhUpdatePtr() { public void handler(osd_bitmap bitmap,int full_refresh) 
	{
		...
		else
		{
			...
			UBytePtr  rom18D = new UBytePtr(memory_region(REGION_USER2)); /*rom 20D starts at offset 0x2000*/
			UBytePtr  prom14D= new UBytePtr(memory_region(REGION_USER3)); /*prom 15D starts at offset 0x100*/
			UBytePtr  rom15F = new UBytePtr(memory_region(REGION_GFX3));  /*rom 15F offs=0x0; 17F offs=0x2000; 18F offs=0x4000; 20F offs=0x6000*/
	
			int scroll_h = magmax_scroll_x.READ_WORD() & 0x3fff;
			int scroll_v = magmax_scroll_y.READ_WORD() & 0xff;
			...
			for (v = 2*8; v < 30*8; v++) /*only for visible area*/
			{
				...
				for (h = 0; h < 256; h++)
				{
					...
					if ((map_h & 0x1) != 0)
						LS273 = rom18B.read( (rom18B_addr + 0x2000) & 0x1fff );		//pixel 1,3,5,7 from rom 20B
					else
