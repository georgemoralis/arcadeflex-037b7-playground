...
import static arcadeflex.WIP.v037b7.drivers.m92.m92_sprite_interrupt;

public class m92
{
	
	static struct_tilemap pf3_wide_layer, pf3_layer, pf2_layer, pf1_wide_layer, pf1_layer, pf1_hlayer;
	public static int[] pf1_control=new int[8],pf2_control=new int[8],pf3_control=new int[8],pf4_control=new int[8];
	public static int pf1_vram_ptr,pf2_vram_ptr,pf3_vram_ptr;
	public static int pf1_enable,pf2_enable,pf3_enable;
	public static int pf1_rowscroll,pf2_rowscroll,pf3_rowscroll;
	public static int pf1_shape,pf2_shape,pf3_shape;
	...
	public static UBytePtr m92_vram_data=new UBytePtr(), m92_spritecontrol=new UBytePtr();
	...
	static int RYPELEO_SPEEDUP() { return m92_game_kludge==1?1:0; }
	...
	public static GetTileInfoPtr get_pf1_tile_info = new GetTileInfoPtr() { public void handler(int tile_index) 
	{
		...
		tile=m92_vram_data.read(tile_index)+(m92_vram_data.read(tile_index+1)<<8);
		color=m92_vram_data.read(tile_index+2);
		SET_TILE_INFO(0,tile,color&0x3f);
	
		if ((m92_vram_data.read(tile_index+3)&1)!=0) pri = 2;
		else if ((color & 0x80) != 0) pri = 1;
		else pri = 0;
	
		tile_info.u32_flags = TILE_FLIPYX((m92_vram_data.read(tile_index+3) & 0x6)>>1) | TILE_SPLIT(pri);
	} };
	...
	public static WriteHandlerPtr m92_master_control_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
		...
		switch (offset) {
			...
			case 7:
				...
				if (m92_raster_machine==0 && m92_raster_enable!=0 && m92_raster_irq_position>128)
	...
	public static VhStartPtr m92_vh_start = new VhStartPtr() { public int handler() 
	{
		if (RYPELEO_SPEEDUP() != 0) {
			...
			pf1_hlayer.u32_transmask[0] = 0xffff;
			pf1_hlayer.u32_transmask[1] = 0x00ff;
			pf1_hlayer.u32_transmask[2] = 0x0001;
		...
		if (pf1_layer==null || pf2_layer==null || pf3_layer==null || pf1_wide_layer==null || pf3_wide_layer==null)
			return 1;
		...
		pf1_layer.u32_transmask[0] = 0xffff;
		pf2_layer.u32_transmask[0] = 0xffff;
		pf3_layer.u32_transmask[0] = 0xffff;
		pf1_wide_layer.u32_transmask[0] = 0xffff;
		pf3_wide_layer.u32_transmask[0] = 0xffff;
		/* split type 1 - pens 0-7 transparent in front half */
		pf1_layer.u32_transmask[1] = 0x00ff;
		pf2_layer.u32_transmask[1] = 0x00ff;
		pf3_layer.u32_transmask[1] = 0x00ff;
		pf1_wide_layer.u32_transmask[1] = 0x00ff;
		pf3_wide_layer.u32_transmask[1] = 0x00ff;
		/* split type 2 - pen 0 transparent in front half */
		pf1_layer.u32_transmask[2] = 0x0001;
		pf2_layer.u32_transmask[2] = 0x0001;
		pf3_layer.u32_transmask[2] = 0x0001;
		pf1_wide_layer.u32_transmask[2] = 0x0001;
		pf3_wide_layer.u32_transmask[2] = 0x0001;
	...
	public static VhUpdatePtr m92_vh_screenrefresh = new VhUpdatePtr() { public void handler(osd_bitmap bitmap,int full_refresh) 
	{
		...
		if (keyboard_pressed_memory(KEYCODE_F1) != 0) {
			m92_raster_enable ^= 1;
	...
	static void m92_screenrefresh(osd_bitmap bitmap, rectangle clip)
	{
