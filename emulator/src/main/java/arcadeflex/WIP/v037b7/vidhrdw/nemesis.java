...
public class nemesis
{
	...
	public static UBytePtr nemesis_xscroll1 = new UBytePtr(), nemesis_xscroll2 = new UBytePtr(), nemesis_yscroll = new UBytePtr();
	public static UBytePtr nemesis_yscroll1 = new UBytePtr(), nemesis_yscroll2 = new UBytePtr();
	
	static osd_bitmap tmpbitmap2;
	static osd_bitmap tmpbitmap3;
	static osd_bitmap tmpbitmap4;
	
	static UBytePtr video1_dirty = new UBytePtr();	/* 0x800 chars - foreground */
	static UBytePtr video2_dirty = new UBytePtr();	/* 0x800 chars - background */
	static UBytePtr char_dirty = new UBytePtr();	/* 2048 chars */
	static UBytePtr sprite_dirty = new UBytePtr();	/* 512 sprites */
	static UBytePtr sprite3216_dirty = new UBytePtr();	/* 256 sprites */
	static UBytePtr sprite816_dirty = new UBytePtr();	/* 1024 sprites */
	static UBytePtr sprite1632_dirty = new UBytePtr();	/* 256 sprites */
	static UBytePtr sprite3232_dirty = new UBytePtr();	/* 128 sprites */
	static UBytePtr sprite168_dirty = new UBytePtr();	/* 1024 sprites */
	static UBytePtr sprite6464_dirty = new UBytePtr();	/* 32 sprites */
        
        static int MULTIPLIER(int bit1, int bit2, int bit3, int bit4, int bit5 ) { 
            return 8 * bit1 + 17 * bit2 + 33 * bit3 + 67 * bit4 + 130 * bit5;
        }
	...
	public static WriteHandlerPtr nemesis_palette_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
		...
		*/
		...
		bit4=(data >>  3)&1;
		bit5=(data >>  4)&1;
		r = MULTIPLIER(bit1,bit2,bit3,bit4,bit5);
		...
		bit4=(data >>  8)&1;
		bit5=(data >>  9)&1;
		g = MULTIPLIER(bit1,bit2,bit3,bit4,bit5);
		...
		bit4=(data >>  13)&1;
		bit5=(data >>  14)&1;
		b = MULTIPLIER(bit1,bit2,bit3,bit4,bit5);
	...
	public static WriteHandlerPtr nemesis_videoram1_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
		...
		if (offset < 0x1000)
			video1_dirty.write(offset / 2, 1);
		else
			video2_dirty.write((offset - 0x1000) / 2, 1);
	} };
...
/*TODO*///		#endif
		...
		if (oldword != newword)
		{
			...
			char_dirty.write(offset / 32, 1);
			sprite_dirty.write(offset / 128, 1);
			sprite3216_dirty.write(offset / 256, 1);
			sprite1632_dirty.write(offset / 256, 1);
			sprite3232_dirty.write(offset / 512, 1);
			sprite168_dirty.write(offset / 64, 1);
			sprite816_dirty.write(offset / 64, 1);
			sprite6464_dirty.write(offset / 2048, 1);
		}
	} };
	...
	public static VhStartPtr nemesis_vh_start = new VhStartPtr() { public int handler() 
	{
		...
		if ((tmpbitmap3 = bitmap_alloc(2 * Machine.drv.screen_width,Machine.drv.screen_height)) == null)
		{
			nemesis_vh_stop.handler();
			return 1;
		}
	
		if ((tmpbitmap4 = bitmap_alloc(2 * Machine.drv.screen_width,Machine.drv.screen_height)) == null)
		{
			nemesis_vh_stop.handler();
			return 1;
		}
	
		char_dirty = new UBytePtr(2048);
		if (char_dirty==null) {
			nemesis_vh_stop.handler();
			return 1;
		}
		...
		sprite3216_dirty = new UBytePtr(256);
		...
		sprite1632_dirty = new UBytePtr(256);
		...
		sprite3232_dirty = new UBytePtr(128);
		...
		sprite168_dirty = new UBytePtr(1024);
		...
		sprite816_dirty = new UBytePtr(1024);
		...
		sprite6464_dirty = new UBytePtr(32);
		...
		video1_dirty = new UBytePtr (0x800);
		video2_dirty = new UBytePtr (0x800);
			...
			rectangle clip,int transparency,int transparent_color,int scale)
	{
		...
		UBytePtr sd=new UBytePtr();
		UBytePtr bm=new UBytePtr();
		int col;
	...
	*/
		...
		paldata = new ShortPtr(gfx.colortable, gfx.color_granularity * color);
			...
			if (zoom != 0xFF || code!=0)
			{
				...
				switch(size&0x38)
				{
					...
					case 0x08:
						...
						char_type=5;
						code/=4;
						if (sprite1632_dirty.read(code) == 1)
						{
							decodechar(Machine.gfx[char_type],code,nemesis_characterram_gfx,
									Machine.drv.gfxdecodeinfo[char_type].gfxlayout);
							sprite1632_dirty.write(code, 0);
					...
					case 0x10:
						...
						char_type=2;
						code/=4;
						if (sprite3216_dirty.read(code) == 1)
						{
							decodechar(Machine.gfx[char_type],code,nemesis_characterram_gfx,
									Machine.drv.gfxdecodeinfo[char_type].gfxlayout);
							sprite3216_dirty.write(code, 0);
						}
						break;
					case 0x18:
						/* sprite 64x64 */
						char_type=7;
						code/=32;
						if (sprite6464_dirty.read(code) == 1)
						{
							decodechar(Machine.gfx[char_type],code,nemesis_characterram_gfx,
									Machine.drv.gfxdecodeinfo[char_type].gfxlayout);
							sprite6464_dirty.write(code, 0);
						}
						break;
					case 0x20:
						/* char 8x8 */
						...
						if (char_dirty.read(code) == 1)
						{
							decodechar(Machine.gfx[char_type],code,nemesis_characterram_gfx,
							Machine.drv.gfxdecodeinfo[char_type].gfxlayout);
							char_dirty.write(code, 0);
						}
						break;
					case 0x28:
						/* sprite 16x8 */
						char_type=6;
						if (sprite168_dirty.read(code) == 1)
						{
							decodechar(Machine.gfx[char_type],code,nemesis_characterram_gfx,
									Machine.drv.gfxdecodeinfo[char_type].gfxlayout);
							sprite168_dirty.write(code, 0);
						}
						break;
					case 0x30:
						/* sprite 8x16 */
						char_type=3;
						if (sprite816_dirty.read(code) == 1)
						{
							decodechar(Machine.gfx[char_type],code,nemesis_characterram_gfx,
									Machine.drv.gfxdecodeinfo[char_type].gfxlayout);
							sprite816_dirty.write(code, 0);
						}
						break;
					default:
					...
					case 0x38:
						...
						char_type=1;
						code/=2;
						if (sprite_dirty.read(code) == 1)
						{
							decodechar(Machine.gfx[char_type],code,nemesis_characterram_gfx,
									Machine.drv.gfxdecodeinfo[char_type].gfxlayout);
							sprite_dirty.write(code, 2);
	...
	static void setup_backgrounds()
	{
		...
		for (offs = 0x1000 - 2;offs >= 0;offs -= 2)
		{
			...
			if (video2_dirty.read(offs/2) != 0 || char_dirty.read(code) != 0)
			{
				int sx,sy,flipx,flipy;
	
				color = nemesis_videoram2.READ_WORD (offs + 0x1000) & 0x7f;
	
				video2_dirty.write(offs/2, 0);
		...
		for (offs = 0x1000 - 2;offs >= 0;offs -= 2)
		{
			...
			if (video1_dirty.read(offs/2)!=0 || char_dirty.read(code)!=0)
			{
				int sx,sy,flipx,flipy;
	
				video1_dirty.write(offs/2, 0);
	...
	public static VhUpdatePtr nemesis_vh_screenrefresh = new VhUpdatePtr() { public void handler(osd_bitmap bitmap,int full_refresh) 
	{
		...
		copyscrollbitmap(bitmap,tmpbitmap2,256,xscroll,0,new int[]{0},Machine.visible_area,TRANSPARENCY_PEN,palette_transparent_pen);
		...
		copyscrollbitmap(bitmap,tmpbitmap4,256,xscroll,0,new int[]{0},Machine.visible_area,TRANSPARENCY_PEN,palette_transparent_pen);
	
		for (offs = 0; offs < 2048; offs++)
		{
			if (char_dirty.read(offs) == 2)
				char_dirty.write(offs, 0);
		}
	} };
	...
	{
		int offs,l;
		int[] xscroll=new int[256],yscroll=new int[256],xscroll2=new int[256],yscroll2=new int[256];
		int culumn_scroll = 0;
		...
		if (nemesis_yscroll.READ_WORD(0x780)!=0 || nemesis_yscroll.READ_WORD(0x790)!=0) {
			...
			copyscrollbitmap(bitmap,tmpbitmap2,0,new int[]{0},128,yscroll,Machine.visible_area,TRANSPARENCY_PEN,palette_transparent_pen);
		} else { /* Rowscroll */
			for (offs = 0;offs < 256;offs++)
			{
				xscroll[offs] = -((nemesis_xscroll1.READ_WORD(2 * offs) & 0xff) +
		...
		if (nemesis_yscroll.READ_WORD(0x700)!=0 || nemesis_yscroll.READ_WORD(0x710)!=0) {
			...
			copyscrollbitmap(bitmap,tmpbitmap,0,new int[]{0},128,yscroll2,Machine.visible_area,TRANSPARENCY_PEN,palette_transparent_pen);
	
			draw_sprites(bitmap);
	
			if (culumn_scroll != 0)
				copyscrollbitmap(bitmap,tmpbitmap4,0,new int[]{0},128,yscroll,Machine.visible_area,TRANSPARENCY_PEN,palette_transparent_pen);
			else
				copyscrollbitmap(bitmap,tmpbitmap4,256,xscroll,0,new int[]{0},Machine.visible_area,TRANSPARENCY_PEN,palette_transparent_pen);
	
			copyscrollbitmap(bitmap,tmpbitmap3,0,new int[]{0},128,yscroll2,Machine.visible_area,TRANSPARENCY_PEN,palette_transparent_pen);
		} else { /* Rowscroll */
			for (offs = 0;offs < 256;offs++)
			{
				xscroll2[offs] = -((nemesis_xscroll2.READ_WORD(2 * offs) & 0xff) +
			...
			copyscrollbitmap(bitmap,tmpbitmap,256,xscroll2,0,new int[]{0},Machine.visible_area,TRANSPARENCY_PEN,palette_transparent_pen);
			...
			copyscrollbitmap(bitmap,tmpbitmap3,256,xscroll2,0,new int[]{0},Machine.visible_area,TRANSPARENCY_PEN,palette_transparent_pen);
		}
