...
public class raiden
{
	...
	public static UBytePtr raiden_back_data = new UBytePtr(), raiden_fore_data = new UBytePtr(), raiden_scroll_ram = new UBytePtr();
	...
	{
		int tile=raiden_back_data.read(2*tile_index)+(raiden_back_data.read(2*tile_index+1)<<8);
		int color=tile >> 12;
	...
	{
		int tile=raiden_fore_data.read(2*tile_index)+(raiden_fore_data.read(2*tile_index+1)<<8);
		int color=tile >> 12;
	...
	public static VhStartPtr raiden_vh_start = new VhStartPtr() { public int handler() 
	{
		...
		if (bg_layer==null || fg_layer==null || tx_layer==null)
			return 1;
	...
	public static VhUpdatePtr raiden_vh_screenrefresh = new VhUpdatePtr() { public void handler(osd_bitmap bitmap,int full_refresh) 
	{
		...
		if (ALTERNATE==0) {
			tilemap_set_scrollx( bg_layer,0, ((raiden_scroll_ram.read(1)<<8)+raiden_scroll_ram.read(0)) );
			tilemap_set_scrolly( bg_layer,0, ((raiden_scroll_ram.read(3)<<8)+raiden_scroll_ram.read(2)) );
			tilemap_set_scrollx( fg_layer,0, ((raiden_scroll_ram.read(5)<<8)+raiden_scroll_ram.read(4)) );
			tilemap_set_scrolly( fg_layer,0, ((raiden_scroll_ram.read(7)<<8)+raiden_scroll_ram.read(6)) );
		}
		else {
			tilemap_set_scrolly( bg_layer,0, ((raiden_scroll_ram.read(0x02)&0x30)<<4)+((raiden_scroll_ram.read(0x04)&0x7f)<<1)+((raiden_scroll_ram.read(0x04)&0x80)>>7) );
			tilemap_set_scrollx( bg_layer,0, ((raiden_scroll_ram.read(0x12)&0x30)<<4)+((raiden_scroll_ram.read(0x14)&0x7f)<<1)+((raiden_scroll_ram.read(0x14)&0x80)>>7) );
			tilemap_set_scrolly( fg_layer,0, ((raiden_scroll_ram.read(0x22)&0x30)<<4)+((raiden_scroll_ram.read(0x24)&0x7f)<<1)+((raiden_scroll_ram.read(0x24)&0x80)>>7) );
			tilemap_set_scrollx( fg_layer,0, ((raiden_scroll_ram.read(0x32)&0x30)<<4)+((raiden_scroll_ram.read(0x34)&0x7f)<<1)+((raiden_scroll_ram.read(0x34)&0x80)>>7) );
		}
