...
public class rampart
{
	...
	static opbase_handlerPtr opbase_override = new opbase_handlerPtr() {
		...
		if (address >= 0x140000 && address < 0x148000)
		{
			...
			OP_RAM = new UBytePtr(slapstic_base, current_bank - 0x140000);
                        OP_ROM = new UBytePtr(slapstic_base, current_bank - 0x140000);
	...
	public static InitDriverPtr init_arcadecr = new InitDriverPtr() { public void handler() 
	{
		...
		UBytePtr temp1 = new UBytePtr(length / 2), temp2 = new UBytePtr(length / 2);
		FILE f;
		int i;
	...
	*/
		for (i = 0; i < length - 8; i += 2, data.inc())
		{
		...
		f = fopen("pgm0.bin", "wb");
		fwrite(temp1.memory, 1, length / 2, f);
		fclose(f);
		f = fopen("pgm1.bin", "wb");
		fwrite(temp2.memory, 1, length / 2, f);
		fclose(f);
