...
import static arcadeflex.WIP.v037b7.vidhrdw.toki.*;
...
public class toki
{
	...
	public static ReadHandlerPtr toki_ports_r  = new ReadHandlerPtr() { public int handler(int offset)
	{
	    switch(offset)
	    {
			case 0:
	...
	public static WriteHandlerPtr toki_soundcommand_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
	...
	static vclk_interruptPtr toki_adpcm_int = new vclk_interruptPtr() {
            @Override
	...
	static MemoryReadAddress readmem[] =
	{
		...
		new MemoryReadAddress( 0x0c0000, 0x0c0005, toki_ports_r ),
		new MemoryReadAddress( 0x0c000e, 0x0c000f, pip_r ),	/* sound related, if we return 0 the code writes */
	...
	***************************************************************************/
	...
	static RomLoadPtr rom_toki2 = new RomLoadPtr(){ public void handler(){ 
		ROM_REGION( 0x60000, REGION_CPU1 );/* 6*64k for 68000 code */
		ROM_LOAD_EVEN( "tokijp.006",   0x00000, 0x20000, 0x03d726b1 );
	...
	static RomLoadPtr rom_toki3 = new RomLoadPtr(){ public void handler(){ 
		ROM_REGION( 0x60000, REGION_CPU1 );/* 6*64k for 68000 code */
	...
	public static InitDriverPtr init_tokib = new InitDriverPtr() { public void handler() 
	{
		...
		if (temp != null)
		{
			for (offs = 0; offs < memory_region_length(REGION_GFX3); offs += 0x20000)
			{
				...
				for (i = 0; i < 16; i++)
				{
					memcpy (new UBytePtr(base, 0x00000 + i * 0x800), new UBytePtr(temp, 0x0000 + i * 0x2000), 0x800);
					memcpy (new UBytePtr(base, 0x10000 + i * 0x800), new UBytePtr(temp, 0x0800 + i * 0x2000), 0x800);
					memcpy (new UBytePtr(base, 0x08000 + i * 0x800), new UBytePtr(temp, 0x1000 + i * 0x2000), 0x800);
					memcpy (new UBytePtr(base, 0x18000 + i * 0x800), new UBytePtr(temp, 0x1800 + i * 0x2000), 0x800);
				}
			}
