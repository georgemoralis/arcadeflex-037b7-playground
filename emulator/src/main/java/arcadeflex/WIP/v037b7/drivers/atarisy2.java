...
import static arcadeflex.WIP.v037b7.cpu.t11.t11.t11_SetBank;

public class atarisy2
{
	public static UBytePtr interrupt_enable = new UBytePtr();
	public static UBytePtr bankselect = new UBytePtr();
	...
	static timer_callback scanline_update = new timer_callback() {
		...
		if (scanline <= Machine.drv.screen_height)
		{
			...
			if ((scanline % 64) == 0)
				if ((interrupt_enable.READ_WORD(0) & 4) != 0)
	...
	public static InterruptPtr vblank_interrupt = new InterruptPtr() { public int handler() 
	{
		...
		if ((interrupt_enable.READ_WORD(0) & 8) != 0)
			atarigen_video_int_gen.handler();
	...
	public static WriteHandlerPtr interrupt_ack_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
		switch (offset & 0x60)
		{
			...
			/* reset VBLANK IRQ */
			case 0x60:
				atarigen_video_int_ack_w.handler(0, 0);
				break;
		}
	} };
	...
	public static WriteHandlerPtr bankselect_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
		int bankoffset[] =
		{
		...
		int oldword = bankselect.READ_WORD(offset);
		int newword = COMBINE_WORD(oldword, data);
		UBytePtr RAM = new UBytePtr(memory_region(REGION_CPU1));
		UBytePtr base = new UBytePtr(RAM, bankoffset[(newword >> 10) & 0x3f]);
	
		bankselect.WRITE_WORD(offset, newword);
		if (offset == 0)
		{
			cpu_setbank(1, base);
	...
	public static ReadHandlerPtr switch_r  = new ReadHandlerPtr() { public int handler(int offset)
	{
		int result = input_port_1_r.handler(offset) | (input_port_2_r.handler(offset) << 8);
	...
	public static ReadHandlerPtr switch_6502_r  = new ReadHandlerPtr() { public int handler(int offset)
	{
		...
		if (has_tms5220==0 || tms5220_ready_r()!=0) result ^= 0x04;
		if ((input_port_2_r.handler(offset) & 0x80)==0) result ^= 0x10;
	...
	public static WriteHandlerPtr adc_strobe_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
		//(void)data;
	...
	public static ReadHandlerPtr sound_r  = new ReadHandlerPtr() { public int handler(int offset)
	{
		...
		return atarigen_sound_r.handler(offset);
	} };
	...
	public static WriteHandlerPtr sound_6502_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
		...
		p2portwr_state = (interrupt_enable.READ_WORD(0) & 2) != 0 ? 1 : 0;
		atarigen_update_interrupts();
	...
	{
		/* clock the state through */
		p2portrd_state = (interrupt_enable.READ_WORD(0) & 1) != 0 ? 1 : 0;
		atarigen_update_interrupts();
	
		/* handle it normally otherwise */
		return atarigen_6502_sound_r.handler(offset);
	} };
	...
	public static WriteHandlerPtr tms5220_strobe_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
		...
		if ((offset & 1)==0 && tms5220_data_strobe!=0)
			if (has_tms5220 != 0)
				tms5220_data_w.handler(0, tms5220_data);
	...
	static MemoryReadAddress main_readmem[] =
	{
		...
		new MemoryReadAddress( 0x1000, 0x11ff, paletteram_word_r ),
		new MemoryReadAddress( 0x1400, 0x1403, adc_r ),
		new MemoryReadAddress( 0x1800, 0x1801, switch_r ),
		new MemoryReadAddress( 0x1c00, 0x1c01, sound_r ),
		new MemoryReadAddress( 0x2000, 0x3fff, atarisys2_videoram_r ),
		new MemoryReadAddress( 0x4000, 0x5fff, MRA_BANK1 ),
		new MemoryReadAddress( 0x6000, 0x7fff, MRA_BANK2 ),
		new MemoryReadAddress( 0x8000, 0x81ff, atarisys2_slapstic_r ),
		new MemoryReadAddress( 0x8200, 0xffff, MRA_ROM ),
	...
	static MemoryWriteAddress main_writemem[] =
	{
		...
		new MemoryWriteAddress( 0x1000, 0x11ff, atarisys2_paletteram_w, paletteram ),
		new MemoryWriteAddress( 0x1400, 0x1403, bankselect_w, bankselect ),
		new MemoryWriteAddress( 0x1480, 0x148f, adc_strobe_w ),
		new MemoryWriteAddress( 0x1580, 0x15ff, interrupt_ack_w ),
		new MemoryWriteAddress( 0x1600, 0x1601, MWA_RAM, interrupt_enable ),
		new MemoryWriteAddress( 0x1680, 0x1681, atarigen_sound_w ),
		new MemoryWriteAddress( 0x1700, 0x1701, atarisys2_hscroll_w, atarigen_hscroll ),
		new MemoryWriteAddress( 0x1780, 0x1781, atarisys2_vscroll_w, atarigen_vscroll ),
		new MemoryWriteAddress( 0x1800, 0x1801, watchdog_reset_w ),
		new MemoryWriteAddress( 0x2000, 0x3fff, atarisys2_videoram_w ),
		new MemoryWriteAddress( 0x4000, 0x7fff, MWA_ROM ),
		new MemoryWriteAddress( 0x8000, 0x81ff, atarisys2_slapstic_w, atarisys2_slapstic ),
		new MemoryWriteAddress( 0x8200, 0xffff, MWA_ROM ),
	...
	static MemoryReadAddress sound_readmem[] =
	{
		...
		new MemoryReadAddress( 0x1810, 0x1813, leta_r ),
		new MemoryReadAddress( 0x1830, 0x183f, pokey2_r ),
		new MemoryReadAddress( 0x1840, 0x1840, switch_6502_r ),
		new MemoryReadAddress( 0x1850, 0x1851, YM2151_status_port_0_r ),
		new MemoryReadAddress( 0x1860, 0x1860, sound_6502_r ),
	...
	{
		new MemoryWriteAddress( 0x0000, 0x0fff, MWA_RAM ),
		new MemoryWriteAddress( 0x1000, 0x17ff, MWA_RAM, atarigen_eeprom, atarigen_eeprom_size ),
		new MemoryWriteAddress( 0x1800, 0x180f, pokey1_w ),
		new MemoryWriteAddress( 0x1830, 0x183f, pokey2_w ),
		new MemoryWriteAddress( 0x1850, 0x1850, YM2151_register_port_0_w ),
		new MemoryWriteAddress( 0x1851, 0x1851, YM2151_data_port_0_w ),
		new MemoryWriteAddress( 0x1870, 0x1870, tms5220_w ),
		new MemoryWriteAddress( 0x1872, 0x1873, tms5220_strobe_w ),
		new MemoryWriteAddress( 0x1874, 0x1874, sound_6502_w ),
		new MemoryWriteAddress( 0x1876, 0x1876, MWA_NOP ),
		new MemoryWriteAddress( 0x1878, 0x1878, atarigen_6502_irq_ack_w ),
		new MemoryWriteAddress( 0x187a, 0x187a, mixer_w ),
		new MemoryWriteAddress( 0x187c, 0x187c, switch_6502_w ),
		new MemoryWriteAddress( 0x187e, 0x187e, sound_enable_w ),
	...
	(
		...
		new MachineCPU[] {
			...
			new MachineCPU(
				...
				null,0,
				atarigen_6502_irq_gen,(int)(1000000000.0/((double)ATARI_CLOCK_20MHz/2/16/16/16/10))
			),
		},
	...
	public static InitDriverPtr init_paperboy = new InitDriverPtr() { public void handler() 
	{
		...
		{
			memcpy(new UBytePtr(memory_region(REGION_CPU1), i + 0x08000), new UBytePtr(memory_region(REGION_CPU1), i), 0x8000);
			memcpy(new UBytePtr(memory_region(REGION_CPU1), i + 0x10000), new UBytePtr(memory_region(REGION_CPU1), i), 0x8000);
			memcpy(new UBytePtr(memory_region(REGION_CPU1), i + 0x18000), new UBytePtr(memory_region(REGION_CPU1), i), 0x8000);
		}
	...
	public static InitDriverPtr init_a720 = new InitDriverPtr() { public void handler() 
	{
	...
	public static InitDriverPtr init_ssprint = new InitDriverPtr() { public void handler() 
	{
		int compressed_default_eeprom[] =
		{
		...
		for (i = 0x10000; i < 0x90000; i += 0x20000)
			memcpy(new UBytePtr(memory_region(REGION_CPU1), i + 0x10000), new UBytePtr(memory_region(REGION_CPU1), i), 0x10000);
	...
	public static InitDriverPtr init_csprint = new InitDriverPtr() { public void handler() 
	{
		int compressed_default_eeprom[] =
		{
	...
	public static InitDriverPtr init_apb = new InitDriverPtr() { public void handler() 
	{
		atarigen_eeprom_default = null;
		slapstic_init(110);
