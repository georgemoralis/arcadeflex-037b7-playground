...
/*TODO*///	
/*TODO*///	
/*TODO*///	extern UINT8 *atarisys1_bankselect;
/*TODO*///	extern UINT8 *atarisys1_prioritycolor;
/*TODO*///	
/*TODO*///	
/*TODO*///	
...
/*TODO*///	
	...
	static timer_entry joystick_timer;
	...
	static atarigen_int_callbackPtr update_interrupts = new atarigen_int_callbackPtr() {
		...
		if (joystick_int!=0 && joystick_int_enable!=0)
			newstate = 2;
	...
	public static InitMachinePtr init_machine = new InitMachinePtr() { public void handler() 
	{
		...
		memset(m6522_regs, 0xff, m6522_regs.length);
...
/*TODO*///		set_led_status(offset,~data & 1);
	...
	public static ReadHandlerPtr joystick_r  = new ReadHandlerPtr() { public int handler(int offset)
	{
		int newval = 0xff;
	
		/* digital joystick type */
		if (joystick_type == 1)
			newval = (input_port_0_r.handler(offset) & (0x80 >> (offset / 2)))!=0 ? 0xf0 : 0x00;
	
		/* Hall-effect analog joystick */
		else if (joystick_type == 2)
			newval = (offset & 2)!=0 ? input_port_0_r.handler(offset) : input_port_1_r.handler(offset);
	
		/* Road Blasters gas pedal */
		else if (joystick_type == 3)
			newval = input_port_1_r.handler(offset);
	...
	public static WriteHandlerPtr joystick_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
		/* the A4 bit enables/disables joystick IRQs */
		joystick_int_enable = ((offset >> 4) & 1) ^ 1;
	} };
	...
	public static ReadHandlerPtr trakball_r  = new ReadHandlerPtr() { public int handler(int offset)
	{
		int result = 0xff;
		...
		if (trackball_type == 1)
		{
			...
			if (which == 0)
			{
				...
				if (player == 0)
				{
					posx = input_port_0_r.handler(offset);
					posy = input_port_1_r.handler(offset);
				}
				else
				{
					posx = input_port_2_r.handler(offset);
					posy = input_port_3_r.handler(offset);
				}
		...
		/* Road Blasters steering wheel */
			...
			result = input_port_0_r.handler(offset);
	...
	{
		int temp = input_port_4_r.handler(offset);
	...
	{
		int temp = input_port_5_r.handler(offset);
	
		if (atarigen_cpu_to_sound_ready != 0) temp ^= 0x08;
		if (atarigen_sound_to_cpu_ready != 0) temp ^= 0x10;
		if ((input_port_4_r.handler(offset) & 0x0040)==0) temp ^= 0x80;
	...
	public static ReadHandlerPtr m6522_r  = new ReadHandlerPtr() { public int handler(int offset)
	{
		...
		{
			case 0x00:	/* DRB */
				return (m6522_drb & m6522_ddrb) | (NOT(tms5220_ready_r()) << 2) | (NOT(tms5220_int_r()) << 3);
	...
	public static WriteHandlerPtr m6522_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
		...
		switch (offset)
		{
			case 0x00:	/* DRB */
				...
				if ((old & 1)==0 && (m6522_drb & 1)!=0)
					tms5220_data_w.handler(0, m6522_dra);
				if ((old & 2)==0 && (m6522_drb & 2)!=0)
					m6522_dra = (m6522_dra & m6522_ddra) | (tms5220_status_r.handler(0) & ~m6522_ddra);
	...
	static opbase_handlerPtr indytemp_setopbase = new opbase_handlerPtr() {
            @Override
		...
		if ((address & 0x80000) != 0)
			atarigen_slapstic_r.handler(0);
		else if ((prevpc & 0x80000) != 0)
			atarigen_slapstic_r.handler(prevpc & 0x7fff);
	...
	{
		new MemoryReadAddress( 0x000000, 0x087fff, MRA_ROM ),
		new MemoryReadAddress( 0x2e0000, 0x2e0003, atarisys1_int3state_r ),
		new MemoryReadAddress( 0x400000, 0x401fff, MRA_BANK1 ),
		new MemoryReadAddress( 0x840000, 0x840003, MRA_BANK2 ),
		new MemoryReadAddress( 0x900000, 0x9fffff, MRA_BANK3 ),
		new MemoryReadAddress( 0xa00000, 0xa01fff, MRA_BANK4 ),
		new MemoryReadAddress( 0xa02000, 0xa02fff, MRA_BANK5 ),
		new MemoryReadAddress( 0xa03000, 0xa03fff, MRA_BANK6 ),
		new MemoryReadAddress( 0xb00000, 0xb007ff, paletteram_word_r ),
		new MemoryReadAddress( 0xf00000, 0xf00fff, atarigen_eeprom_r ),
		new MemoryReadAddress( 0xf20000, 0xf20007, trakball_r ),
		new MemoryReadAddress( 0xf40000, 0xf4001f, joystick_r ),
		new MemoryReadAddress( 0xf60000, 0xf60003, input_r ),
		new MemoryReadAddress( 0xfc0000, 0xfc0003, atarigen_sound_r ),
	...
	{
		new MemoryWriteAddress( 0x000000, 0x087fff, MWA_ROM ),
		new MemoryWriteAddress( 0x400000, 0x401fff, MWA_BANK1 ),
		new MemoryWriteAddress( 0x800000, 0x800003, atarisys1_hscroll_w, atarigen_hscroll ),
		new MemoryWriteAddress( 0x820000, 0x820003, atarisys1_vscroll_w, atarigen_vscroll ),
		new MemoryWriteAddress( 0x840000, 0x840003, MWA_BANK2, atarisys1_prioritycolor ),
		new MemoryWriteAddress( 0x860000, 0x860003, atarisys1_bankselect_w, atarisys1_bankselect ),
		new MemoryWriteAddress( 0x880000, 0x880003, watchdog_reset_w ),
		new MemoryWriteAddress( 0x8a0000, 0x8a0003, atarigen_video_int_ack_w ),
		new MemoryWriteAddress( 0x8c0000, 0x8c0003, atarigen_eeprom_enable_w ),
		new MemoryWriteAddress( 0x900000, 0x9fffff, MWA_BANK3 ),
		new MemoryWriteAddress( 0xa00000, 0xa01fff, atarisys1_playfieldram_w, atarigen_playfieldram, atarigen_playfieldram_size ),
		new MemoryWriteAddress( 0xa02000, 0xa02fff, atarisys1_spriteram_w, atarigen_spriteram, atarigen_spriteram_size ),
		new MemoryWriteAddress( 0xa03000, 0xa03fff, MWA_BANK6, atarigen_alpharam, atarigen_alpharam_size ),
		new MemoryWriteAddress( 0xb00000, 0xb007ff, paletteram_IIIIRRRRGGGGBBBB_word_w, paletteram ),
		new MemoryWriteAddress( 0xf00000, 0xf00fff, atarigen_eeprom_w, atarigen_eeprom, atarigen_eeprom_size ),
		new MemoryWriteAddress( 0xf40000, 0xf4001f, joystick_w ),
		new MemoryWriteAddress( 0xfe0000, 0xfe0003, atarigen_sound_w ),
	...
	static MemoryReadAddress sound_readmem[] =
	{
		...
		new MemoryReadAddress( 0x1000, 0x100f, m6522_r ),
		new MemoryReadAddress( 0x1800, 0x1801, YM2151_status_port_0_r ),
		new MemoryReadAddress( 0x1810, 0x1810, atarigen_6502_sound_r ),
		new MemoryReadAddress( 0x1820, 0x1820, switch_6502_r ),
		new MemoryReadAddress( 0x1870, 0x187f, pokey1_r ),
	...
	static MemoryWriteAddress sound_writemem[] =
	{
		...
		new MemoryWriteAddress( 0x1000, 0x100f, m6522_w ),
		new MemoryWriteAddress( 0x1800, 0x1800, YM2151_register_port_0_w ),
		new MemoryWriteAddress( 0x1801, 0x1801, YM2151_data_port_0_w ),
		new MemoryWriteAddress( 0x1810, 0x1810, atarigen_6502_sound_w ),
		new MemoryWriteAddress( 0x1824, 0x1825, led_w ),
		new MemoryWriteAddress( 0x1820, 0x1827, MWA_NOP ),
		new MemoryWriteAddress( 0x1870, 0x187f, pokey1_w ),
	...
	static void rom_decode()
	{
		...
		for (i = 0; i < chips; i++, data.inc(0x2000))
		{
