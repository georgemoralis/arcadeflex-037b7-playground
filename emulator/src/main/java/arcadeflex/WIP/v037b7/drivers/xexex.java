...
import static arcadeflex.WIP.v037b7.vidhrdw.xexex.*;
...
public class xexex
{
	...
	public static ReadHandlerPtr K053247_scattered_word_r  = new ReadHandlerPtr() { public int handler(int offset)
	{
		...
		else
		{
			...
			return K053247_word_r.handler(offset);
		}
	} };
	...
	//	logerror("%04x: write %02x to spriteram %04x\n",cpu_get_pc(),data,offset);
			K053247_word_w.handler(offset,data);
		}
	} };
	...
	public static InterruptPtr xexex_interrupt = new InterruptPtr() { public int handler() 
	{
		switch (cpu_getiloops())
		{
			...
			case 1:
				if (K053246_is_IRQ_enabled()!=0 && (cur_control2 & 0x0040)!=0)
					return 5;
				break;
	
			case 2:
				if (K053246_is_IRQ_enabled()!=0 && (cur_control2 & 0x0020)!=0)
					return 6;
				break;
		}
	...
	{
		cpu_setbank(3, new UBytePtr(memory_region(REGION_CPU2), 0x10000 + (data&7)*0x2000));
	} };
	...
	{
		if ((cur_back_ctrla & 1)==0)
			logerror("Back: Reading rom memory with enable=0\n");
		return (new UBytePtr(memory_region(REGION_GFX3), 2048*cur_back_select + (offset>>2))).read();
	} };
...
/*TODO*///		new MemoryReadAddress( 0x8000, 0xbfff, MRA_BANK3 ),
/*TODO*///		new MemoryReadAddress( 0xc000, 0xdf7f, MRA_RAM ),
/*TODO*///		new MemoryReadAddress( 0xe000, 0xe22f, MRA_RAM ),
/*TODO*///		new MemoryReadAddress( 0xec01, 0xec01, YM2151_status_port_0_r ),
/*TODO*///		new MemoryReadAddress( 0xf002, 0xf002, sound_cmd_r ),
...
/*TODO*///		new MemoryWriteAddress( 0xc000, 0xdf7f, MWA_RAM ),
/*TODO*///		new MemoryWriteAddress( 0xe000, 0xe22f, MWA_RAM ),
/*TODO*///		new MemoryWriteAddress( 0xec00, 0xec00, YM2151_register_port_0_w ),
/*TODO*///		new MemoryWriteAddress( 0xec01, 0xec01, YM2151_data_port_0_w ),
/*TODO*///		new MemoryWriteAddress( 0xf000, 0xf000, sound_status_w ),
/*TODO*///		new MemoryWriteAddress( 0xf800, 0xf800, sound_bankswitch_w ),
...
/*TODO*///			new MachineCPU(
/*TODO*///				CPU_Z80,
/*TODO*///				2000000,	/* 2 MHz ? (xtal is 32MHz/19.432Mhz) */
