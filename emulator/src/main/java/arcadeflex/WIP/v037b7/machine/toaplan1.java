...
import static arcadeflex.WIP.v037b7.drivers.toaplan1.toaplan1_sharedram;
...
public class toaplan1
{
	...
	public static int toaplan1_coin_count; /* coin count increments on startup ? , so dont count it */
	
	public static int toaplan1_int_enable;
	static int unk;
	static int credits;
	static int latch;
	static int dsp_execute;
	static int dsp_addr_w, main_ram_seg;
	...
	public static ReadHandlerPtr demonwld_dsp_r  = new ReadHandlerPtr() { public int handler(int offset)
	{
		...
		switch (main_ram_seg) {
			case 0xc00000:	input_data = cpu_bankbase[1].READ_WORD(((dsp_addr_w))); break;
	...
	public static WriteHandlerPtr demonwld_dsp_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
		...
		if (offset == 1) {
			...
			dsp_execute = 0;
			switch (main_ram_seg) {
				case 0xc00000:	cpu_bankbase[1].WRITE_WORD(((dsp_addr_w)),data);
	...
	{
/*TODO*///	#if 0
/*TODO*///		logerror("68000:%08x  Writing %08x to %08x.\n",cpu_get_pc() ,data ,0xe0000a + offset);
/*TODO*///	#endif
	...
	public static WriteHandlerPtr toaplan1_coin_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
		...
		/* The following are coin counts after coin-lock active (faulty coin-lock ?) */
			...
			case 0xec: coin_lockout_global_w.handler(0,0); break;	/* ??? count games played */
