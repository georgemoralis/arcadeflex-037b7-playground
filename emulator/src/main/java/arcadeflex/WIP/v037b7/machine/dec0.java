...
import static arcadeflex.WIP.v037b7.vidhrdw.dec0.*;
import static arcadeflex.WIP.v037b7.drivers.dec0.dec0_ram;
...
public class dec0
{
	...
	public static ReadHandlerPtr dec0_controls_r  = new ReadHandlerPtr() { public int handler(int offset)
	{
		switch (offset)
		{
	...
	public static ReadHandlerPtr dec0_rotary_r  = new ReadHandlerPtr() { public int handler(int offset)
	{
		switch (offset)
		{
	...
	public static ReadHandlerPtr midres_controls_r  = new ReadHandlerPtr() { public int handler(int offset)
	{
		switch (offset)
		{
	...
	public static ReadHandlerPtr slyspy_controls_r  = new ReadHandlerPtr() { public int handler(int offset)
	{
		switch (offset)
		{
			case 0: /* Dip Switches */
	...
	public static ReadHandlerPtr slyspy_protection_r  = new ReadHandlerPtr() { public int handler(int offset)
	{
	...
	*/
	...
	public static WriteHandlerPtr slyspy_state_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
		slyspy_state=0;
	} };
	
	public static ReadHandlerPtr slyspy_state_r  = new ReadHandlerPtr() { public int handler(int offset)
	{
		slyspy_state++;
	...
	public static WriteHandlerPtr slyspy_240000_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
		switch (slyspy_state) {
			case 0x3:
				dec0_pf1_data_w.handler(offset,data);
				return;
			case 0x2:
				dec0_pf2_data_w.handler(offset,data);
				return;
			case 0x0:
				if (offset<0x10) dec0_pf2_control_0_w.handler(offset,data);
				else if (offset<0x20) dec0_pf2_control_1_w.handler(offset-0x10,data);
				return;
		}
		logerror("Wrote to 240000 %02x at %04x %04x (Trap %02x)\n",offset,cpu_get_pc(),data,slyspy_state);
	} };
	
	public static WriteHandlerPtr slyspy_242000_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
		switch (slyspy_state) {
			case 0x2: /* Trap A */
				dec0_pf1_data_w.handler(offset,data);
				return;
			case 0x0: /* Trap C */
				if (offset<0x80) dec0_pf2_colscroll_w.handler(offset,data);
				else if (offset<0x600) dec0_pf2_rowscroll_w.handler(offset-0x400,data);
				return;
		}
		logerror("Wrote to 242000 %02x at %04x %04x (Trap %02x)\n",offset,cpu_get_pc(),data,slyspy_state);
	} };
	
	public static WriteHandlerPtr slyspy_246000_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
		switch (slyspy_state) {
			case 0x0:
				dec0_pf2_data_w.handler(offset,data);
	...
	public static WriteHandlerPtr slyspy_248000_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
		switch (slyspy_state) {
			case 0x1:
				dec0_pf1_data_w.handler(offset,data);
			...
			case 0x3:
				...
				return;
			case 0x0:
				if (offset<0x10) dec0_pf1_control_0_w.handler(offset,data);
				else if (offset<0x20) dec0_pf1_control_1_w.handler(offset-0x10,data);
				return;
		}
		logerror("Wrote to 248000 %02x at %04x %04x (Trap %02x)\n",offset,cpu_get_pc(),data,slyspy_state);
	} };
	
	public static WriteHandlerPtr slyspy_24c000_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
		switch (slyspy_state) {
			case 0x1: /* Trap 9 */
				dec0_pf2_data_w.handler(offset,data);
				return;
			case 0x0: /* Trap C */
				if (offset<0x80) dec0_pf1_colscroll_w.handler(offset,data);
				else if (offset<0x600) dec0_pf1_rowscroll_w.handler(offset-0x400,data);
				return;
		}
		logerror("Wrote to 24c000 %02x at %04x %04x (Trap %02x)\n",offset,cpu_get_pc(),data,slyspy_state);
	} };
	
	public static WriteHandlerPtr slyspy_24e000_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
		switch (slyspy_state) {
			case 0x2:
			case 0x0:
	...
	public static ReadHandlerPtr hippodrm_68000_share_r  = new ReadHandlerPtr() { public int handler(int offset)
	{
		if (offset==0) cpu_yield(); /* A wee helper */
		return share[offset/2];
	} };
	
	public static WriteHandlerPtr hippodrm_68000_share_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
	...
	*/
	...
	static timer_entry i8751_timer;
	
	public static timer_callback i8751_callback = new timer_callback() { public void handler(int param) 
	{
	...
	public static void dec0_i8751_write(int data)
	{
	...
	*/
	...
	public static void dec0_i8751_reset()
	{
		i8751_return=0;
	}
	...
	public static ReadHandlerPtr hbarrel_cycle_r  = new ReadHandlerPtr() { public int handler(int offset)
	{
		if (cpu_get_pc()==0x130ca) {cpu_spinuntil_int(); return 0;} return dec0_ram.READ_WORD(0x10);
	} };
	
	public static ReadHandlerPtr hbarrelu_cycle_r  = new ReadHandlerPtr() { public int handler(int offset)
	{
		if (cpu_get_pc()==0x131a0) {cpu_spinuntil_int(); return 0;} return dec0_ram.READ_WORD(0x10);
	} };
	...
	static void hbarrel_custom_memory()
	{
	...
	RAM.WRITE_WORD (0xb3e,0x8008);
	}
	}
	
	static void hbarrelu_custom_memory()
	{
	...
	{ /* Remove this patch once processing time of i8751 is simulated */
	UBytePtr RAM = new UBytePtr(memory_region(REGION_CPU1));
	RAM.WRITE_WORD (0xb68,0x8008);
	}
	}
	
	static void robocop_custom_memory()
	{
		/* To do */
	}
	
	static void hippodrm_custom_memory()
	{
	...
	public static void dec0_custom_memory()
	{
		...
		if (strcmp(Machine.gamedrv.name,"drgninja")==0)	GAME=2;
		if (strcmp(Machine.gamedrv.name,"birdtry")==0) 	GAME=3;
		if (strcmp(Machine.gamedrv.name,"robocop")==0) robocop_custom_memory();
		if (strcmp(Machine.gamedrv.name,"hippodrm")==0) hippodrm_custom_memory();
		if (strcmp(Machine.gamedrv.name,"ffantasy")==0) hippodrm_custom_memory();
	}
}
