...
public class snk {
    ...
    static int MAX_VRAM_SIZE = (64 * 64 * 2);
    ...
    static int GFX_CHARS = 0;
    static int GFX_TILES = 1;
    ...
    public static VhConvertColorPromPtr ikari_vh_convert_color_prom = new VhConvertColorPromPtr() {
        public void handler(char[] palette, char[] colortable, UBytePtr color_prom) {
             ...
             */
            for (i = 0; i < 256; i += 8) {
                palette[p_inc + i * 3 + 0] = palette[p_inc + i * 3 + 1] = palette[p_inc + i * 3 + 2] = 14;
            }
        }
    };
 ...
 /* to be moved to memmap */
    ...
    static void ikari_draw_background(osd_bitmap bitmap, int xscroll, int yscroll) {
        ...
        for (offs = 0; offs < 32 * 32 * 2; offs += 2) {
            ...
            if (tile_number != dirtybuffer[offs]
                ...
                dirtybuffer[offs] = (char) (tile_number & 0xFF);
                dirtybuffer[offs + 1] = (char) (attributes & 0xFF);
    ...
    public static VhUpdatePtr tdfever_vh_screenrefresh = new VhUpdatePtr() {
        public void handler(osd_bitmap bitmap, int full_refresh) {
            ...
            {
                /*unsigned*/ char bg_attributes = ram.read(0xc880);
            ...
            {
                /*unsigned*/ char sprite_attributes = ram.read(0xc900);
        ...
        public void handler(osd_bitmap bitmap, int full_refresh) {
...

            {
                ...
                if (gwar_sprite_placement != 0) /* gwar */ {
                    if ((bg_attributes & 0x10) != 0) {
                        sp16_y += 256;
                    }
                    if ((bg_attributes & 0x40) != 0) {
                        sp16_x += 256;
                    }
                    if ((bg_attributes & 0x20) != 0) {
                        sp32_y += 256;
                    }
                    if ((bg_attributes & 0x80) != 0) {
                        sp32_x += 256;
                    }
                } else {
                    /* psychos, bermudet, chopper1... */
 /*unsigned*/ char spp_attributes = ram.read(0xca80);
                    if ((spp_attributes & 0x04) != 0) {
                        sp16_y += 256;
                    }
                    if ((spp_attributes & 0x08) != 0) {
                        sp32_y += 256;
                    }
                    if ((spp_attributes & 0x10) != 0) {
                        sp16_x += 256;
                    }
                    if ((spp_attributes & 0x20) != 0) {
                        sp32_x += 256;
                    }
                }
