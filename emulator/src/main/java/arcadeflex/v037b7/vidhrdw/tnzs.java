...
public class tnzs {
     ...
     * *************************************************************************
     *
     * The New Zealand Story doesn't have a color PROM. It uses 1024 bytes of
     * RAM to dynamically create the palette. Each couple of bytes defines one
     ...
     **************************************************************************
     */
    public static VhConvertColorPromPtr arkanoi2_vh_convert_color_prom = new VhConvertColorPromPtr() {
        public void handler(char[] palette, char[] colortable, UBytePtr color_prom) {
            int i, col;
            int p_ptr = 0;
            for (i = 0; i < Machine.drv.total_colors; i++) {
                col = (color_prom.read(i) << 8) + color_prom.read(i + 512);
                palette[p_ptr++] = (char) (((col & 0x7c00) >> 7) & 0xFF);/* Red */
                palette[p_ptr++] = (char) (((col & 0x03e0) >> 2) & 0xFF);/* Green */
                palette[p_ptr++] = (char) (((col & 0x001f) << 3) & 0xFF);/* Blue */
            }
        }
    };
     ...
     **************************************************************************
     */
    public static VhStartPtr tnzs_vh_start = new VhStartPtr() {
        public int handler() {
            int column, x, y;
            for (column = 0; column < 16; column++) {
                if ((tnzs_column[column] = bitmap_alloc(32, 256)) == null) {
     ...
     **************************************************************************
     */
    public static VhStopPtr tnzs_vh_stop = new VhStopPtr() {
        public void handler() {
            int column;
            ...
            for (y = 0; y < 16; y++) {
                for (x = 0; x < 2; x++, i++) {
                    ...
                    if (tnzs_dirty_map[column * 2 + x][y] != tile) {
                        ...
                        color = (m.read(i + 0x1200) & 0xf8) >> 3;
                        /* colours at d600-d7ff */
                        sx = x * 16;
                        sy = y * 16;
 ...
 /* one we expect at the beginning of a level (2C vs. 2A). */
        ...
        upperbits = (tnzs_scrollram.read(0x102) + tnzs_scrollram.read(0x103) * 256) & 0xFFFFFFFFL;
        /* again, it's not clear why there are two areas, but Arkanoid 2 uses these */
 /* for the end of game animation */
        upperbits = (upperbits | tnzs_scrollram.read(0x182) + tnzs_scrollram.read(0x183) * 256) & 0xFFFFFFFFL;

        for (column = 0; column < tot; column++) {
            scrollx = (int) (tnzs_scrollram.read(column * 16 + 4) - ((upperbits & 0x01) * 256));
            if (tnzs_screenflip != 0) {
            ...
            upperbits = (upperbits >> 1) & 0xFFFFFFFFL;
        }
    }
    ...
    public static VhUpdatePtr arkanoi2_vh_screenrefresh = new VhUpdatePtr() {
        public void handler(osd_bitmap bitmap, int full_refresh) {
		   ...
		   (I'm not 100% sure about this) */
            ...
            tnzs_vh_draw_foreground(bitmap,
                    ...
                    new UBytePtr(tnzs_objram, 0x1200));
            /* color : d200 */
        }
    };

    public static VhUpdatePtr tnzs_vh_screenrefresh = new VhUpdatePtr() {
        public void handler(osd_bitmap bitmap, int full_refresh) {
