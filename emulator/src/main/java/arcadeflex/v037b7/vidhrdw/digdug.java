...
public class digdug {
    ...
    public static UBytePtr digdug_vlatches = new UBytePtr();
    ...
    public static VhConvertColorPromPtr digdug_vh_convert_color_prom = new VhConvertColorPromPtr() {
        public void handler(char[] palette, char[] colortable, UBytePtr color_prom) {
            ...
            /* sprites */
            for (i = 0 * 4; i < 64 * 4; i++) {
                colortable[8 * 2 + i] = (char) (31 - ((color_prom.read(i + 32) & 0x0f) + 0x10));
            }
            /* playfield */
            for (i = 64 * 4; i < 128 * 4; i++) {
                colortable[8 * 2 + i] = (char) (31 - (color_prom.read(i + 32) & 0x0f));
            }
        }
    };
    ...
    public static WriteHandlerPtr digdug_flipscreen_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
    ...
    public static VhUpdatePtr digdug_vh_screenrefresh = new VhUpdatePtr() {
        public void handler(osd_bitmap bitmap, int full_refresh) {
            ...
            if (playenable != 0) {
                ...
                pfindex = playfield;
                pfcolor = playcolor;
                pf = new UBytePtr(memory_region(REGION_GFX4), (pfindex << 10));
            }
