...
import static arcadeflex.v037b7.sound.k005289H.*;
    ...
    public static StreamInitPtr K005289_update = new StreamInitPtr() {
        public void handler(int chip, ShortPtr buffer, int length) {
           ...
           // memset(mixer_buffer, 0, length * 2/*sizeof(INT16)*/);
            ...
            if (v != 0 && f != 0) {
                UBytePtr w = new UBytePtr(channel_list[0].wave);
                ...
                for (i = 0; i < length; i++) {
                    ...
                    //*mix++ += ((w[offs] & 0x0f) - 8) * v;
                    short _w = mix.read(0);
                    mix.write(0, (short) Math.abs((_w + (short) (((w.read(offs) & 0x0f) - 8) * v))));
                    mix.offset += 2;
                }
            ...
            if (v != 0 && f != 0) {
                UBytePtr w = new UBytePtr(channel_list[1].wave);
            ...
            // added by Chuso
            mix.offset=0;
            //length=mix.memory.length;
            length=get_lowest((buffer.memory.length), mixer_lookup.memory.length, mix.memory.length);
            mixer_lookup.offset=0;
            buffer.offset=0;
            // END added by Chuso
            
            for (i = 0; i < (length/2); i++) {
                //*buffer++ = mixer_lookup[*mix++];
/*TODO*///                buffer.write(0, (short) mixer_lookup.read(mix.read(0)));
                buffer.inc();
                mix.inc();
            }
        }

        private int get_lowest(int i, int length, int length0) {
            int _out=i;
    ...
    public int start(MachineSound msound) {
        ...
        sound_prom = new UBytePtr(memory_region(intf.region));
