...
public class commonH {
 ...
 /* the macros below. */
    public static final int ROMFLAG_MASK = 0xfc000000;/* 6 bits worth of flags in the high nibble */
 ...
 /* Masks for individual ROMs */
    ...
    public static final int ROMFLAG_OPTIONAL = 0x04000000;/* Optional ROM, not needed for basic emulation */
    ...
    /* load only if sound_old emulation is turned on */
    public static int BADCRC(int crc) {
        return ~crc;
    }
    public static RomModule[] rommodule_macro = null;
    public static ArrayList<RomModule> arload = new ArrayList<RomModule>();

    /* start of memory region */
    public static void ROM_REGION(int length, int type) {
        arload.add(new RomModule(null, length, 0, type));
    }

    /* ROM to load */
    public static void ROM_LOAD(String name, int offset, int length, int crc) {
        arload.add(new RomModule(name, offset, length, crc));
    }
    ...
    public static void ROM_RELOAD_NIB_LOW(int offset, int length) {
        arload.add(new RomModule("-1", offset, (length) | ROMFLAG_NIBBLE, 0));
    }

    public static void ROM_RELOAD_NIB_HIGH(int offset, int length) {
        arload.add(new RomModule("1", offset, (length) | ROMFLAG_NIBBLE | ROMFLAG_ALTERNATE, 0));
    }
 ...
 /* load the ROM at even/odd addresses. Useful with 16 bit games */
    ...
    public static void ROM_RELOAD_WIDE(int offset, int length) {
        arload.add(new RomModule("-1", offset, (length) | ROMFLAG_WIDE, 0));
    }
    ...
    public static void ROM_RELOAD_WIDE_SWAP(int offset, int length) {
        arload.add(new RomModule("-1", offset, (length) | ROMFLAG_WIDE | ROMFLAG_SWAP, 0));
    }

    /* Data is split between 4 roms, always use this in groups of 4! */
    public static void ROM_LOAD_QUAD(String name, int offset, int length, int crc) {
        arload.add(new RomModule(name, offset, length | ROMFLAG_QUAD, crc));
    }

    public static void ROM_LOAD_OPTIONAL(String name, int offset, int length, int crc) {
        arload.add(new RomModule(name, offset, length | ROMFLAG_OPTIONAL, crc));
    }

    public static void ROM_LOAD_V20_EVEN(String name, int offset, int length, int crc) {
        ROM_LOAD_EVEN(name, offset, length, crc);
    }

    public static void ROM_RELOAD_V20_EVEN(int offset, int length) {
        ROM_RELOAD_EVEN(offset, length);
    }

    public static void ROM_LOAD_V20_ODD(String name, int offset, int length, int crc) {
        ROM_LOAD_ODD(name, offset, length, crc);
    }

    public static void ROM_RELOAD_V20_ODD(int offset, int length) {
        ROM_RELOAD_ODD(offset, length);
    }

    public static void ROM_LOAD_V20_WIDE(String name, int offset, int length, int crc) {
    ...
    public static void ROM_END() {
        arload.add(new RomModule(null, 0, 0, 0));
