...
public class memoryH {
 ...
 /* 24 bits address (word access - byte granularity) */
    ...
    public static final int ABITS_MIN_24 = 0;/* minimum memory block is 1 byte */
 /* 24 bits address (big endian - word access) */
    public static final int ABITS1_24BEW = 15;
    public static final int ABITS2_24BEW = 8;
    public static final int ABITS_MIN_24BEW = 1;/* minimum memory block is 2 bytes */
 /* 26 bits address (little endian - dword access) */
    public static final int ABITS1_26LEW = 14;
    public static final int ABITS2_26LEW = 10;
    public static final int ABITS_MIN_26LEW = 2;/* minimum memory block is 4 bytes */
 ...
 /* 32 bits address (little endian dword access) */
    public static final int ABITS1_32LEW = 23;
    public static final int ABITS2_32LEW = 8;
    public static final int ABITS_MIN_32LEW = 1;/* minimum memory block is 2 bytes */
 /* mask bits */
    public static int MHMASK(int abits) {
        return (0xffffffff >>> (32 - abits));//probably return unsigned??(shadow)
    }
...
/*TODO*///#define WRITE_WORD(a,d) 	  (*(UINT16 *)(a) = (d))
    public static int COMBINE_WORD(int w, int d) {
        return (((w) & ((d) >> 16)) | ((d) & 0xffff));//(((w) & ((d) >> 16)) | ((d) & 0xffff))
    }

    public static void COMBINE_WORD_MEM(UBytePtr a, int offset, int d) {
        a.WRITE_WORD(offset, (a.READ_WORD(offset) & ((d) >> 16) | d));//(WRITE_WORD((a), (READ_WORD(a) & ((d) >> 16)) | (d)))
    }
