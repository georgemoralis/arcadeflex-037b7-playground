...
public class rainbow {
     ...
     ************************************
     */
    public static InterruptPtr rainbow_interrupt = new InterruptPtr() {
        public int handler() {
            return 4;
            /* Interrupt vector 4 */
        }
     ...
     ************************************
     */
    public static WriteHandlerPtr rainbow_c_chip_w = new WriteHandlerPtr() {
     ...
     ************************************
     */
    public static ReadHandlerPtr rainbow_c_chip_r = new ReadHandlerPtr() {
        public int handler(int offset) {
            UBytePtr CROM = memory_region(REGION_USER1);
            /* C-Chip Dump */
            ...
            switch (offset + 1) {
                ...
                case 0x007:
                    if (FrameBank == 0) {
                        ans = input_port_2_r.handler(offset);
                    }
                    break;

                case 0x009:
                    if (FrameBank == 0) {
                        ans = input_port_3_r.handler(offset);
                    }
                    break;

                case 0x00b:
                    if (FrameBank == 0) {
                        ans = input_port_4_r.handler(offset);
                    }
                    break;

                case 0x00d:
                    if (FrameBank == 0) {
                        ans = input_port_5_r.handler(offset);
                    }
                    break;
