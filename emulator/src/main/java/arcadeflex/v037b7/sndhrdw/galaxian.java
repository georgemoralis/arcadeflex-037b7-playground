...
public class galaxian {
        ...
        public void handler(int ch, ShortPtr buffer, int length) {
            ...
            if (pitch != 0xff) {
                for (i = 0; i < length; i++) {
                    ...
                    buffer.writeinc((short) ((mix << 8) / STEPS));
                }
            } else {
                for (i = 0; i < length; i++) {
    ...
    public static WriteHandlerPtr galaxian_pitch_w = new WriteHandlerPtr() {
    ...
    public static WriteHandlerPtr galaxian_vol_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
            stream_update(tone_stream, 0);
    ...
    public static WriteHandlerPtr galaxian_noise_enable_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
    ...
    public static WriteHandlerPtr galaxian_shoot_enable_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
            if ((data & 1) != 0 && (last_port2 & 1) == 0) {
                if (shootsampleloaded != 0) {
                    ...
                    mixer_play_sample_16(channelshoot, new ShortPtr(shootwave), shoot_length, shoot_rate, 0);
    ...
    public static ShStartPtr galaxian_sh_start = new ShStartPtr() {
        public int handler(MachineSound msound) {
            ...
            if ((noisewave = new short[(int) NOISE_LENGTH * 2]) == null) {
                return 1;
            }
            ...
            if ((shootwave = new short[shoot_length * 2]) == null) {
                noisewave = null;
                return 1;
            }
 ...
 /*
		...
		And,  duty will be changed,too.
                ...
                /* key on */
                int IC8Lcnt = (int) (SHOOT_KEYON_TIME * shoot_rate);
                /* count for key off */
 ...
 /*       connection : NOISE - R46(10K) - C29(0.1u) - R48(2.2K) - C28 */
                ...
                double c29K1 = (shoot_rate) != 0 ? Math.exp(-1 / (22000 * 0.00000001) / shoot_rate) : 0;
                /* form C28   */
                double c29K2 = (shoot_rate) != 0 ? Math.exp(-1 / (100000 * 0.00000001) / shoot_rate) : 0;
                /* from noise */
 /* NE555 timer */
 ...
 /* key off */
            ...
            if (deathsampleloaded == 0) {
                ...
                mixer_play_sample_16(channelnoise, new ShortPtr(noisewave), (int) NOISE_LENGTH, (int) NOISE_RATE, 1);
            }
            if (shootsampleloaded == 0) {
                mixer_set_volume(channelshoot, 0);
                mixer_play_sample_16(channelshoot, new ShortPtr(shootwave), SHOOT_LENGTH, SHOOT_RATE, 1);
            }

            mixer_set_volume(channellfo + 0, 0);
            mixer_play_sample_16(channellfo + 0, new ShortPtr(backgroundwave), sizeof(backgroundwave), 1000, 1);
            mixer_set_volume(channellfo + 1, 0);
            mixer_play_sample_16(channellfo + 1, new ShortPtr(backgroundwave), sizeof(backgroundwave), 1000, 1);
            mixer_set_volume(channellfo + 2, 0);
            mixer_play_sample_16(channellfo + 2, new ShortPtr(backgroundwave), sizeof(backgroundwave), 1000, 1);
    ...
    public static WriteHandlerPtr galaxian_lfo_freq_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
