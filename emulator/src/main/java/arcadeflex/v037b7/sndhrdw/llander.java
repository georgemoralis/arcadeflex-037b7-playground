...
import static gr.codebb.arcadeflex.old.sound.mixer.mixer_play_streamed_sample_16;
...
public class llander {
 ...
 /* multipler is held as a fixed point number 16:16                                                */
            ...
            lfsr_index = 0;
            if ((lfsr_buffer = new char[65536 * 2]) == null) {
                return 1;
            }
            ...
            for (loop = 0; loop < 65536; loop++) {
                ...
                nor1 = (!(bit14 != 0 && bit6 != 0)) ? 0 : 1;
                /* Note the inversion for the NOR gate */
            ...
            if ((sample_buffer = new ShortPtr(2 * buffer_len)) == null) {
                return 1;
            }
     ...
     * <p>
     * 3khz (tone) Gain 1 6khz (tone) Gain 1 thrust (12khz noise) Gain 2
     * explosion (12khz noise) Gain 4
     * <p>
     * After combining the sources the output is scaled accordingly. (Div 8)
     * <p>
     * Sound generation is done by oversampling (x64) of the signal to remove
     * the need to interpolate between samples. It gives the closest point of
     * the sample point to the real signal, there is a small error between the
     * two, the higher the oversample rate the lower the error.
     * <p>
     * oversample_rate oversample_number = sample_number * ---------------
     * sample_rate
     * <p>
     ...
     * <p>
     * The calculations are all done in fixed point 16.16 format. The oversample
     * is mapped to the sinewave in the following manner
     * <p>
     ...
     * modelled with an array, the array value at the current index points to
     * the next index to be used, the table is precalulated at startup.
     * <p>
     * The output of noise is taken evertime we cross a 12khz boundary, the code
     * then sets a target value (noisetarg), we then scan the gap between the
     * last oversample point and the current oversample point at the oversample
     * rate using the following algorithm:
     * <p>
     * noisecurrent = noisecurrent + (noisetarg-noisecurrent) * small_value
     * <p>
     * (currently small value = 1/256)
     * <p>
     * again this is done in fixed point 16.16 and results in the smoothing of
     * the output waveform which reduces the high frequency noise. It also
     * reduces the overall amplitude swing of the output, hence the gain
     * partitioning.
     * <p>
     * You could probably argue that the oversample rate could be dropped
     * without any loss in quality and would recude the cpu load, but lander is
     * hardly a cpu hog.
     * <p>
     * The outputs from all of the above are then added and scaled up/down to a
     * single sample
     * <p>
     * K.Wilkins 13/5/98
     **************************************************************************
 ...
 /* each step takes us 1/256 of the difference between desired and current */
            for (loop2 = lastoversampnum; loop2 < oversampnum; loop2++) {
                noisecurrent += (noisetarg - noisecurrent) >> 7;
                /* Equiv of multiply by 1/256 */
            }
    ...
    public static ShUpdatePtr llander_sh_update_partial = new ShUpdatePtr() {
        public void handler() {
            ...
            newpos = sound_scalebufferpos(buffer_len);
