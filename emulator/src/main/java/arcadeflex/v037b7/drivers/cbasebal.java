...
import static gr.codebb.arcadeflex.WIP.v037b7.mame.palette.paletteram_xxxxBBBBRRRRGGGG_w;

public class cbasebal {

    static int rambank;

    public static WriteHandlerPtr cbasebal_bankswitch_w = new WriteHandlerPtr() {
 ...
 /* bits 6-7 select RAM bank */
    ...
    public static ReadHandlerPtr bankedram_r = new ReadHandlerPtr() {
        public int handler(int offset) {
            if (rambank == 2) {
                return cbasebal_textram_r.handler(offset);
                /* VRAM */
            } else if (rambank == 1) {
                if (offset < 0x800) {
                ...
                return cbasebal_scrollram_r.handler(offset);
                /* SCROLL */
            }
        }
    };
    ...
    public static WriteHandlerPtr bankedram_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
            if (rambank == 2) {
                cbasebal_textram_w.handler(offset, data);
            } else if (rambank == 1) {
                if (offset < 0x800) {
                    paletteram_xxxxBBBBRRRRGGGG_w.handler(offset, data);
                }
            } else {
                cbasebal_scrollram_w.handler(offset, data);
            }
        }
    };
    ...
    public static InitDriverPtr init_cbasebal = new InitDriverPtr() {
        public void handler() {
            pang_decode();
        }
    };
