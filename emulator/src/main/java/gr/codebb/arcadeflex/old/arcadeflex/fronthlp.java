...
import static arcadeflex.v037b7.mame.common.showdisclaimer;
import static arcadeflex.v037b7.mame.cpuintrf.cputype_name;
import static arcadeflex.v037b7.mame.commonH.ROMFLAG_MASK;
import static arcadeflex.v037b7.mame.commonH.RomModule;
import static arcadeflex.v037b7.mame.sndintrf.sound_name;
import static arcadeflex.v037b7.mame.sndintrf.sound_num;
...
public class fronthlp {
  ...
  public static int frontend_help(int argc, String[] argv) {
    ...
    for (i = 1; i < argc; i++) {
      ...
      if (stricmp(argv[i], "-listdetails") == 0)
        list = LIST_LISTDETAILS; /* A detailed MAMELIST.TXT type roms lister */
      ...
      if (strlen(gamename) > 0) {
        ...
        if (stricmp(argv[i], "-verifysets") == 0)
          verify = VERIFY_ROMS | VERIFY_VERBOSE | VERIFY_TERSE;
    ...
    if (help != 0) /* brief help - useful to get current version info */ {
      ...
      showdisclaimer();
      printf(
          "Usage:  java -jar arcadeflex.jar gamename [options]\n"
    ...
    switch (list) /* front-end utilities ;) */ {
      ...
      case LIST_LISTSAMDIR: /* games list with samples directories */
        ...
        while (drivers[i] != null) {
          ...
          /*TODO*/ ) /*&& !strwildcmp(gamename, drivers[i].name)*/) {
            ...
            // *)drivers[i].drv.sound[j].sound_interface).samplenames;
            /*TODO*/
      ...
      case LIST_LISTSAMPLES: /* game samples list */
        j = 0;
        while (drivers[j] != null && (stricmp(gamename, drivers[j].name) != 0)) j++;
        if (drivers[j] == null) {
        ...
        if (list == LIST_LISTROMS) {
          ...
          /*TODO*/
          //					samplenames = ((struct Samplesinterface
          // *)gamedrv.drv.sound[k].sound_interface).samplenames;
          /*TODO*/
          //					if (samplenames != 0 && samplenames[0] != 0)
          /*TODO*/
          //					{
      ...
      case LIST_LMR:
        {
          ...
          total = 0;
          for (i = 0; drivers[i] != null; i++) total++;
        ...
        while (drivers[i] != null) {
          ...
          /*TODO*/ ) /*&& !strwildcmp(gamename, drivers[i].name)*/) {
            ...
            for (j = 0; j < MAX_CPU; j++) {
              ...
              else printf("%-8s ", cputype_name(x_cpu[j].cpu_type));
            }
...
/*TODO*/
      ...
      case LIST_WRONGORIENTATION: /* list drivers which incorrectly use the orientation and visible area fields */
        while (drivers[i] != null) {
          ...
          //						printf("%s %dx%d\n",drivers[i].name,
          /*TODO*/
          //								drivers[i].drv.visible_area.max_x - drivers[i].drv.visible_area.min_x +
          // 1,
          /*TODO*/
          //								drivers[i].drv.visible_area.max_y - drivers[i].drv.visible_area.min_y +
          // 1);
          /*TODO*/
          //				}
          i++;
      ...
      case LIST_LISTCRC: /* list all crc-32 */
        ...
        while (drivers[i] != null) {
          ...
          while (romp != null
              && (romp[romp_ptr].name != null
                  || romp[romp_ptr].offset != 0
                  || romp[romp_ptr].length != 0)) {
          ...
          while (romp != null
            ...
            if (romp[romp_ptr].name != null
              ...
              while (drivers[j] != null) {
                ...
                int romp1_ptr = 0;
                while (romp1 != null
                    && (romp1[romp1_ptr].name != null
                        || romp1[romp1_ptr].offset != 0
                        || romp1[romp1_ptr].length != 0)) {
                  if (romp1[romp1_ptr].name != null
                      && romp1[romp1_ptr].name != "-1"
    ...
    if (verify != 0) /* "verify" utilities */ {
      ...
      //			{
      /*TODO*/
      //				res = VerifyRomSet (i,(verify & VERIFY_TERSE) ? terse_printf :
      // (verify_printf_proc)printf);
      ...
      //		{
      /*TODO*/
      //			printf ("%s ", (verify & VERIFY_ROMS)!=0 ? "romset" : "sampleset" );
      /*TODO*/
      //			if (notfound > 0)
      /*TODO*/
      //				printf("\"%8s\" not found!\n",gamename);
      ...
      /*TODO*/
      //					(verify & VERIFY_ROMS)!=0? "romsets" : "samplesets", correct);
      /*TODO*/
      //			if (incorrect > 0)
      /*TODO*/
      //				return 2;
