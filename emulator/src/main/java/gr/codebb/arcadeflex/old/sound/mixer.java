 ...
 */
package gr.codebb.arcadeflex.old.sound;
...
import static arcadeflex.v037b7.mame.sndintrf.sound_scalebufferpos;
...
public class mixer {
    ...
    public static class mixer_channel_data {
        ...
        int/*UINT8*/ is_stream;
        int/*UINT8*/ is_playing;
        int/*UINT8*/ is_looping;
        int/*UINT8*/ is_16bit;
    ...
    /* 16-bit mix buffers */
    static short[] mix_buffer = new short[ACCUMULATOR_SAMPLES * 2];
    /* *2 for stereo */
        ...
        while (samples_to_generate > 0) {
            ...
            if (is_stereo == 0 || channel.pan == MIXER_PAN_LEFT) {
                ...
                //throw new UnsupportedOperationException("unsupported");
                		while (source.offset < source_end && samples_to_generate > 0)
				{
					int mixing_value = source.read() * mixing_volume;
					left_accum[output_pos] += mixing_value;
					right_accum[output_pos] += mixing_value;
					input_frac += step_size;
					source.offset += (int)( input_frac >> FRACTION_BITS );
