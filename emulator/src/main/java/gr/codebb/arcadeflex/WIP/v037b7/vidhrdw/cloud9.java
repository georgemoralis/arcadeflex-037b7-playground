...
public class cloud9
{
	...
	***************************************************************************/
	public static WriteHandlerPtr cloud9_bitmap_regs_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
		...
		if (offset == 2)
		{
			...
			if (y >= 0x0c)
			{
				UBytePtr vptr = new UBytePtr(1024*128);
				int vpixel=0;
	...
	***************************************************************************/
	static void redraw_bitmap()
	{
		...
		int cloud9_both_banks_save = cloud9_both_banks.read();
		int cloud9_vram_bank_save  = cloud9_vram_bank.read();
	
		cloud9_both_banks.write(0, 0);
	
		for (offs = 0; offs < videoram_size[0]; offs++)
		{
			cloud9_vram_bank.write(0, 0);
			cloud9_bitmap_w.handler(offs, videoram.read(offs));
	
			cloud9_vram_bank.write(0, 0x80);
			cloud9_bitmap_w.handler(offs, cloud9_vram2.read(offs));
		}
	
		cloud9_both_banks.write(0, cloud9_both_banks_save);
		cloud9_vram_bank.write(0, cloud9_vram_bank_save);
	}
	
	
	public static VhUpdatePtr cloud9_vh_screenrefresh = new VhUpdatePtr() { public void handler(osd_bitmap bitmap,int full_refresh) 
	{
		int offs;
