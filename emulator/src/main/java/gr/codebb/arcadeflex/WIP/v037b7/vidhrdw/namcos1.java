...
import static gr.codebb.arcadeflex.WIP.v037b7.mame.drawgfx.gfx_drawmode_table;
...
import static arcadeflex.v037b7.mame.driverH.GAME_REQUIRES_16BIT;
...
/*TODO*///	#endif
	...
	static GetTileInfoPtr background_get_info = new GetTileInfoPtr() {
	...
	static GetTileInfoPtr foreground_get_info = new GetTileInfoPtr() {
            public void handler(int tile_index) {
                int code = info_vram.read(tile_index+1)+((info_vram.read(tile_index)&0x3f)<<8);
                SET_TILE_INFO(1,code,info_color);
                tile_info.mask_data = new UBytePtr(mask_ptr, code);
