...
public class victory
{
	...
	static osd_bitmap fgbitmap;
	static UBytePtr bgdirty=new UBytePtr();
	static UBytePtr chardirty=new UBytePtr();
	static UBytePtr scandirty=new UBytePtr();
	static UBytePtr rram=new UBytePtr(), gram=new UBytePtr(), bram=new UBytePtr();
	...
	/* if it's not 8 */
	public static final int MICRO_STATE_CLOCK_PERIOD	= (int) TIME_IN_HZ(11827000/8);
	
	
	/* debugging constants */
	public static final int LOG_MICROCODE		= 0;
	public static final int LOG_COLLISION		= 0;
	...
	static void victory_update_irq()
	{
			...
			for (y = 0; y < ycount; y++)
			{
				...
				if ((micro.cmd & 0x08)==0 || fgcoll!=0)
				{
					...
					if ((micro.cmd & 0x40) != 0)
					{
						...
						rram.write(dstoffs + 1, (rram.read(dstoffs + 1)) ^ src << nshift);
					}
				}
	...
	*/
		...
		for (y = 0; y < 256; y++)
			if (scandirty.read(y) != 0)
			{
				for (x = 0; x < 256; x += 8)
				{
					...
					plot_pixel.handler(fgbitmap, x + 0, y, ((r & 0x80) >> 5) | ((b & 0x80) >> 6) | ((g & 0x80) >> 7));
					plot_pixel.handler(fgbitmap, x + 1, y, ((r & 0x40) >> 4) | ((b & 0x40) >> 5) | ((g & 0x40) >> 6));
					plot_pixel.handler(fgbitmap, x + 2, y, ((r & 0x20) >> 3) | ((b & 0x20) >> 4) | ((g & 0x20) >> 5));
					plot_pixel.handler(fgbitmap, x + 3, y, ((r & 0x10) >> 2) | ((b & 0x10) >> 3) | ((g & 0x10) >> 4));
					plot_pixel.handler(fgbitmap, x + 4, y, ((r & 0x08) >> 1) | ((b & 0x08) >> 2) | ((g & 0x08) >> 3));
					plot_pixel.handler(fgbitmap, x + 5, y, ((r & 0x04)     ) | ((b & 0x04) >> 1) | ((g & 0x04) >> 2));
					plot_pixel.handler(fgbitmap, x + 6, y, ((r & 0x02) << 1) | ((b & 0x02)     ) | ((g & 0x02) >> 1));
					plot_pixel.handler(fgbitmap, x + 7, y, ((r & 0x01) << 2) | ((b & 0x01) << 1) | ((g & 0x01)     ));
				}
				scandirty.write(y, 0);
			}
	}
	...
	public static timer_callback bgcoll_irq_callback = new timer_callback() { public void handler(int param) 
	{
		bgcollx = param & 0xff;
		bgcolly = param >> 8;
		bgcoll = 1;
	...
	public static VhEofCallbackPtr victory_vh_eof = new VhEofCallbackPtr() {
		...
		for (y = 0; y < 256; y++)
		{
			...
			if (fgbitmap.depth == 8)
			{
				UBytePtr fg = new UBytePtr(fgbitmap.line[sy], 0);
				UBytePtr bg = new UBytePtr(bgbitmap.line[y], 0);
			...
			{
				UShortArray fg = new UShortArray(fgbitmap.line[sy], 0);
				UShortArray bg = new UShortArray(bgbitmap.line[y], 0);
	...
	public static VhUpdatePtr victory_vh_screenrefresh = new VhUpdatePtr() { public void handler(osd_bitmap bitmap,int full_refresh) 
	{
		int bgcollmask = (video_control & 4)!=0 ? 4 : 7;
		int count = 0;
		int x, y;
		...
		for (y = 0; y < 256; y++)
		{
			...
			if (bitmap.depth == 8)
			{
				UBytePtr fg = new UBytePtr(fgbitmap.line[y], 0);
				UBytePtr bg = new UBytePtr(bgbitmap.line[sy], 0);
				UBytePtr dst = new UBytePtr(bitmap.line[y], 0);
	
				for (x = 0; x < 256; x++)
				{
					int fpix = fg.readinc();
					int bpix = bg.read((x + scrollx) & 255);
					dst.writeinc( Machine.pens[bpix | (fpix << 3)] );
					if (fpix!=0 && (bpix & bgcollmask)!=0 && count++ < 128)
						timer_set(pixel_time(x, y), x | (y << 8), bgcoll_irq_callback);
				}
			}
			...
			{
				UShortArray fg = new UShortArray(fgbitmap.line[y], 0);
				UShortArray bg = new UShortArray(bgbitmap.line[sy], 0);
				UShortArray dst = new UShortArray(bitmap.line[y], 0);
				...
				for (x = 0; x < 256; x++)
				{
					int fpix = fg.read();
                                        fg.offset++;
					int bpix = bg.read((x + scrollx) & 255);
					dst.write(0, Machine.pens[bpix | (fpix << 3)] );
                                        dst.offset++;
					if (fpix!=0 && (bpix & bgcollmask)!=0 && count++ < 128)
						timer_set(pixel_time(x, y), x | (y << 8), bgcoll_irq_callback);
				}
