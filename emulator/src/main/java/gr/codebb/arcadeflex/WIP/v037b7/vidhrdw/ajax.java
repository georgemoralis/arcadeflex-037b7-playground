...
public class ajax {
    ...
    public static int u8_ajax_priority;
    ...
    public static K051960_callbackProcPtr sprite_callback = new K051960_callbackProcPtr() {
        public void handler(int[] code, int[] color, int[] priority, int[] shadow) {
		   ...
		   never over F
            ...
            /* F = 8 */
            if ((color[0] & 0x10) != 0) {
                priority[0] |= 0xf0f0;	/* Z = 4 */
            }
            if ((~color[0] & 0x40) != 0) {
                priority[0] |= 0xcccc;	/* A = 2 */
            }
            if ((color[0] & 0x20) != 0) {
                priority[0] |= 0xaaaa;	/* B = 1 */
            }
     ...
     **************************************************************************
     */
    public static VhStartPtr ajax_vh_start = new VhStartPtr() {
        public int handler() {
    ...
    public static VhStopPtr ajax_vh_stop = new VhStopPtr() {
        public void handler() {
            K052109_vh_stop();
            K051960_vh_stop();
            K051316_vh_stop_0();
     ...
     **************************************************************************
     */
    public static VhUpdatePtr ajax_vh_screenrefresh = new VhUpdatePtr() {
