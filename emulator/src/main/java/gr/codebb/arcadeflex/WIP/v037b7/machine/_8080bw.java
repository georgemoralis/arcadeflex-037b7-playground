...
public class _8080bw {
    ...
    public static ReadHandlerPtr invaders_shift_data_rev_r = new ReadHandlerPtr() {
        public int handler(int offset) {
    ...
    public static ReadHandlerPtr invaders_shift_data_comp_r = new ReadHandlerPtr() {
        public int handler(int offset) {
            return (((((shift_data1 << 8) | shift_data2) << (shift_amount & 0x07)) >> 8) & 0xff) ^ 0xff;
        }
    };
     ...
     * **************************************************************************
     */
    public static ReadHandlerPtr boothill_shift_data_r = new ReadHandlerPtr() {
        public int handler(int offset) {
            if (shift_amount < 0x10) {
                return invaders_shift_data_r.handler(0);
            } else {
                return invaders_shift_data_rev_r.handler(0);
            }
        }
    };

    /* Grays binary again! */
    static int boothill_controller_table[]
            = {
                0x00, 0x40, 0x60, 0x70, 0x30, 0x10, 0x50, 0x50
            };

    public static ReadHandlerPtr boothill_port_0_r = new ReadHandlerPtr() {
        public int handler(int offset) {
            return (input_port_0_r.handler(0) & 0x8f) | boothill_controller_table[input_port_3_r.handler(0) >> 5];
        }
    };

    public static ReadHandlerPtr boothill_port_1_r = new ReadHandlerPtr() {
        public int handler(int offset) {
            return (input_port_1_r.handler(0) & 0x8f) | boothill_controller_table[input_port_4_r.handler(0) >> 5];
        }
    };

    static int gunfight_controller_table[]
            = {
                0x10, 0x50, 0x70, 0x30, 0x20, 0x60, 0x40, 0x00
            };

    public static ReadHandlerPtr gunfight_port_0_r = new ReadHandlerPtr() {
        public int handler(int offset) {
            return (input_port_0_r.handler(0) & 0x8f) | (gunfight_controller_table[input_port_3_r.handler(0) >> 5]);
        }
    };

    public static ReadHandlerPtr gunfight_port_1_r = new ReadHandlerPtr() {
        public int handler(int offset) {
     ...
     */
    static int graybit6_controller_table[]
            = {
                0, 1, 3, 2, 6, 7, 5, 4,
    ...
    public static ReadHandlerPtr spcenctr_port_0_r = new ReadHandlerPtr() {
        public int handler(int offset) {
            return (input_port_0_r.handler(0) & 0xc0) + (graybit6_controller_table[input_port_0_r.handler(0) & 0x3f] ^ 0x3f);
        }
    };

    public static ReadHandlerPtr spcenctr_port_1_r = new ReadHandlerPtr() {
        public int handler(int offset) {
            return (input_port_1_r.handler(0) & 0xc0) + (graybit6_controller_table[input_port_1_r.handler(0) & 0x3f] ^ 0x3f);
        }
    };

    public static ReadHandlerPtr seawolf_port_1_r = new ReadHandlerPtr() {
        public int handler(int offset) {
    ...
    public static ReadHandlerPtr desertgu_port_1_r = new ReadHandlerPtr() {
        public int handler(int offset) {
            return readinputport(desertgu_controller_select != 0 ? 0 : 2);
        }
    };

    public static WriteHandlerPtr desertgu_controller_select_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
