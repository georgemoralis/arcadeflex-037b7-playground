...
public class gberet {
     ...
     *
     * Green Beret has a 32 bytes palette PROM and two 256 bytes color lookup
     * table PROMs (one for sprites, one for characters). The palette PROM is
    ...
    public static GetTileInfoPtr get_tile_info = new GetTileInfoPtr() {
        ...
        public void handler(int tile_index) {
            int u8_attr = gberet_colorram.read(tile_index);
            SET_TILE_INFO(0, gberet_videoram.read(tile_index) + ((u8_attr & 0x40) << 2), u8_attr & 0x0f);
            tile_info.u32_flags = TILE_FLIPYX((u8_attr & 0x30) >> 4) | TILE_SPLIT((u8_attr & 0x80) >> 7);
        }
    };
     ...
     **************************************************************************
     */
    public static VhStartPtr gberet_vh_start = new VhStartPtr() {
        public int handler() {
            ...
            bg_tilemap.u32_transmask[0] = 0x0001;
            /* split type 0 has pen 1 transparent in front half */
            bg_tilemap.u32_transmask[1] = 0xffff;
            /* split type 1 is totally transparent in front half */
            tilemap_set_scroll_rows(bg_tilemap, 32);
            return 0;
        }
    };

    public static InitDriverPtr init_gberet = new InitDriverPtr() {
        public void handler() {
            sprites_type = 0;
        }
    };

    public static InitDriverPtr init_gberetb = new InitDriverPtr() {
        public void handler() {
            sprites_type = 1;
        }
    };
     ...
     **************************************************************************
     */
    public static WriteHandlerPtr gberet_videoram_w = new WriteHandlerPtr() {
    ...
    public static WriteHandlerPtr gberet_colorram_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
    ...
    public static WriteHandlerPtr gberet_e044_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
    ...
    public static WriteHandlerPtr gberet_scroll_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
            int scroll;
    ...
    public static WriteHandlerPtr gberetb_scroll_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
            if (offset != 0) {
                data |= 0x100;
            }
    ...
    public static InterruptPtr gberet_interrupt = new InterruptPtr() {
        public int handler() {
            if (cpu_getiloops() == 0) {
    ...
    public static VhUpdatePtr gberet_vh_screenrefresh = new VhUpdatePtr() {
