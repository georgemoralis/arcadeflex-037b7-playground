...
public class rastan {
 ...
 /* status of soundboard ( reports any commands pending ) */
    static int/*unsigned char*/ u8_SlaveContrStat = 0;
    ...
    static int/*unsigned char*/ u8_soundcommand;
    static int/*unsigned char*/ u8_soundboarddata;
	...
	** it must be said to stop instead. This is the address that does it.
    ...
    static int/*unsigned char*/ u8_pom = 0;
            ...
            if (transmit == 0) {
                ...
                if (transmit == 0) {
                    if (tr_mode == 2) {
                        ...
                        u8_SlaveContrStat |= 4;
                        /* report data pending on main */
                        cpu_spin();
                        /* writing should take longer than emulated, so spin */
                    } else {
    ...
    /*TODO*///	public static WriteHandlerPtr rastan_adpcm_trigger_w = new WriteHandlerPtr() {public void handler(int offset, int data)
/*TODO*///	{
/*TODO*///		UBytePtr rom = memory_region(REGION_SOUND1);
/*TODO*///		int len = memory_region_length(REGION_SOUND1);
/*TODO*///		int start = data << 8;
/*TODO*///		int end;
/*TODO*///	
/*TODO*///		/* look for end of sample */
/*TODO*///		end = (start + 3) & ~3;
/*TODO*///		while (end < len && *((UINT32 *)(&rom[end])) != 0x08080808)
/*TODO*///			end += 4;
/*TODO*///	
...
/*TODO*///	} };
    ...
    public static ReadHandlerPtr rastan_sound_comm_r = new ReadHandlerPtr() {
        public int handler(int offset) {
            ...
            m_transmit--;
            if (m_tr_mode == 2) {
                u8_SlaveContrStat &= 0xfb;
                /* clear pending data for main bit */
