...
import static arcadeflex.v037b7.mame.common.set_vh_global_attribute;
import static arcadeflex.common.ptrLib.*;
import static arcadeflex.v037b7.mame.osdependH.osd_bitmap;
import static arcadeflex.v037b7.mame.common.*;
import static gr.codebb.arcadeflex.WIP.v037b7.mame.mame.Machine;
import static arcadeflex.v037b7.generic.funcPtr.*;
import static arcadeflex.v037b7.mame.commonH.REGION_USER1;
import static arcadeflex.v037b7.mame.commonH.flip_screen;
import static arcadeflex.v037b7.mame.drawgfxH.*;
import static gr.codebb.arcadeflex.WIP.v037b7.vidhrdw.cosmic.cosmicg_map_color;
import static gr.codebb.arcadeflex.WIP.v037b7.vidhrdw.cosmic.magspot2_map_color;
import static gr.codebb.arcadeflex.WIP.v037b7.vidhrdw.cosmic.panic_map_color;
...
public class cosmic {
    ...
    public abstract static interface map_colorPtr {

        public abstract int handler(int x, int y);
    }
    public static map_colorPtr map_color;
 ...
 /* this one seems to be OK for the start of the game       */
    static short nomnlnd_tree_positions[][]
            = {
                {66, 63}, {66, 159}
            };

    static short nomnlnd_water_positions[][]
            = {
    ...
    static map_colorPtr panic_map_color = new map_colorPtr() {
        public int handler(int x, int y) {
            /* 8 x 16 coloring */
            char _byte = memory_region(REGION_USER1).read(color_base + (x / 16) * 32 + (y / 8));
    ...
    static map_colorPtr cosmicg_map_color = new map_colorPtr() {
        public int handler(int x, int y) {
            char _byte;
    ...
    static map_colorPtr magspot2_map_color = new map_colorPtr() {
        public int handler(int x, int y) {
            char _byte;
    ...
    public static VhConvertColorPromPtr cosmica_vh_convert_color_prom = new VhConvertColorPromPtr() {
        public void handler(char[] palette, char[] colortable, UBytePtr color_prom) {
            ...
            for (i = 0; i < TOTAL_COLORS(0) / 2; i++) {
                colortable[Machine.drv.gfxdecodeinfo[0].color_codes_start + i] = (char) (color_prom.read() & 0x07);
                colortable[Machine.drv.gfxdecodeinfo[0].color_codes_start + i + (TOTAL_COLORS(0) / 2)] = (char) (((color_prom.readinc()) >> 4) & 0x07);
            }
        ...
        public void handler(char[] palette, char[] colortable, UBytePtr color_prom) {
            ...
            for (i = 0; i < Machine.drv.total_colors; i++) {
                if (i > 8) {
                    ...
                    palette[p_ptr++] = (char) (0xaa * ((i >> 0) & 1));
                }

                palette[p_ptr++] = (char) (0xaa * ((i >> 1) & 1));
                palette[p_ptr++] = (char) (0xaa * ((i >> 2) & 1));
            }
 ...
 /* 16 colors, 8 sprite color codes				  */
    ...
    public static WriteHandlerPtr nomnlnd_background_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
            nomnlnd_background_on = data;
        }
    };

    public static WriteHandlerPtr cosmica_videoram_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
            int i, x, y, col;
    ...
    public static VhUpdatePtr cosmicg_vh_screenrefresh = new VhUpdatePtr() {
    ...
    public static VhUpdatePtr panic_vh_screenrefresh = new VhUpdatePtr() {
        public void handler(osd_bitmap bitmap, int full_refresh) {
            int offs;
    ...
    public static VhUpdatePtr cosmica_vh_screenrefresh = new VhUpdatePtr() {
        public void handler(osd_bitmap bitmap, int full_refresh) {
            int offs;
    ...
    public static VhUpdatePtr magspot2_vh_screenrefresh = new VhUpdatePtr() {
        public void handler(osd_bitmap bitmap, int full_refresh) {
            int offs;
    ...
    static /*UINT8*/ int u8_water_animate = 0;
    public static VhUpdatePtr nomnlnd_vh_screenrefresh = new VhUpdatePtr() {
        public void handler(osd_bitmap bitmap, int full_refresh) {
            int offs;
            ...
            if (nomnlnd_background_on != 0) {
                ...
                u8_water_animate = (u8_water_animate + 1) & 0xFF;
