...
import static gr.codebb.arcadeflex.WIP.v037b7.cpu.h6280.h6280H.H6280_INT_IRQ1;
...
public class battlera {
    ...
    static UBytePtr HuC6270_vram;
    static int[] u8_tile_dirty;
    static int[] u8_sprite_dirty;
    static int[] u8_vram_dirty;
    static osd_bitmap tile_bitmap, front_bitmap;
     ...
     * ***************************************************************************
     */
    public static WriteHandlerPtr battlera_palette_w = new WriteHandlerPtr() {
     ...
     * ***************************************************************************
     */
    static void draw_sprites(osd_bitmap bitmap, rectangle clip, int pri) {
        ...
        for (offs = (HuC6270_registers[19] << 1) + 0x200 - 8; offs >= (HuC6270_registers[19] << 1); offs -= 8) {
            if ((HuC6270_vram.read(offs + 7) & 0x80) != 0 && pri == 0) {
                continue;
            }
            if ((HuC6270_vram.read(offs + 7) & 0x80) == 0 && pri != 0) {
                continue;
            ...
            if (cgx != 0 && cgy == 2) {
                code = code & 0x3fc; /* Title screen */
            }

            if (fx != 0 && cgx != 0) {
                code2 = code;
                code++;
            } /* Swap tile order on X flips */ else {
                code2 = code + 1;
            }
    ...
    static void screenrefresh(osd_bitmap bitmap, rectangle clip) {
        ...
        for (offs = 0; offs < 256; offs++) {
            if ((offs % 16) == 0) {
                palette_used_colors.write(offs, PALETTE_COLOR_TRANSPARENT);
            } else {
                palette_used_colors.write(offs, PALETTE_COLOR_USED);
            }
        }
        ...
        for (offs = 0x0000; offs < 0x2000; offs += 2) {
            ...
            if (u8_tile_dirty[code] != 0 || u8_vram_dirty[offs / 2] != 0) {
                u8_vram_dirty[offs / 2] = 0;
        ...
        for (code = 0x0000; code < 0x1000; code++) {
            u8_tile_dirty[code] = 0;
        }
        for (code = 0x0000; code < 0x400; code++) {
            u8_sprite_dirty[code] = 0;
        }
 ...
 /* Render low priority sprites, if enabled */
        if (sb_enable != 0) {
            draw_sprites(bitmap, clip, 0);
        }

        /* Render background over sprites */
        copyscrollbitmap(bitmap, front_bitmap, 1, new int[]{scrollx}, 1, new int[]{scrolly}, clip, TRANSPARENCY_PEN, palette_transparent_pen);

        /* Render high priority sprites, if enabled */
        if (sb_enable != 0) {
            draw_sprites(bitmap, clip, 1);
        }
    }
     ...
     * ***************************************************************************
     */
    public static ReadHandlerPtr HuC6270_register_r = new ReadHandlerPtr() {
        public int handler(int offset) {
            int rr;

            if ((current_scanline + 56) == HuC6270_registers[6]) {
                rr = 1;
            } else {
                rr = 0;
            }
    ...
    public static WriteHandlerPtr HuC6270_register_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
            switch (offset) {
                case 0:
                    /* Select data region */
     ...
     * ***************************************************************************
     */
    public static ReadHandlerPtr HuC6270_data_r = new ReadHandlerPtr() {
        public int handler(int offset) {
            int result;
    ...
    public static WriteHandlerPtr HuC6270_data_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
            switch (offset) {
                case 0:
                    /* LSB */
                    switch (VDC_register) {
                        ...
                        case 2:
                            ...
                            if (HuC6270_vram.read((HuC6270_registers[0] << 1) | 1) != data) {
                                ...
                                u8_tile_dirty[HuC6270_registers[0] >> 4] = 1;
                                u8_sprite_dirty[HuC6270_registers[0] >> 6] = 1;
                            }
                            if (HuC6270_registers[0] < 0x1000) {
                                u8_vram_dirty[HuC6270_registers[0]] = 1;
                            }
                            return;
 ...
 /* Bits 4,5 unknown (EX) */
                 ...
                 */
                case 1:
                    /* MSB (Autoincrement on this write) */
                    switch (VDC_register) {
 ...
 /* IW - Auto-increment values */
     ...
     * ***************************************************************************
     */
    public static VhUpdatePtr battlera_vh_screenrefresh = new VhUpdatePtr() {
        public void handler(osd_bitmap bitmap, int full_refresh) {
            /* Nothing */
        }
    };

    public static void partial_refresh(osd_bitmap bitmap, int current_line) {
    ...
    public static void battlera_vh_raster_partial_refresh(osd_bitmap bitmap, int start_line, int end_line) {
        rectangle clip = new rectangle();
    ...
    public static InterruptPtr battlera_interrupt = new InterruptPtr() {
        public int handler() {
            current_scanline = 255 - cpu_getiloops();
            /* 8 lines clipped at top */
 ...
 /* If raster interrupt occurs, refresh screen _up_ to this point */
     ...
     * ***************************************************************************
     */
    public static ReadHandlerPtr HuC6270_debug_r = new ReadHandlerPtr() {
        public int handler(int offset) {
            return HuC6270_vram.read(offset);
        }
    };

    public static WriteHandlerPtr HuC6270_debug_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
     ...
     * ***************************************************************************
     */
    public static VhStopPtr battlera_vh_stop = new VhStopPtr() {
        public void handler() {
            u8_tile_dirty = null;
            HuC6270_vram = null;
            u8_sprite_dirty = null;
            u8_vram_dirty = null;
    ...
    public static VhStartPtr battlera_vh_start = new VhStartPtr() {
        public int handler() {
            HuC6270_vram = new UBytePtr(0x20000);
            u8_tile_dirty = new int[0x1000];
            u8_sprite_dirty = new int[0x400];
            u8_vram_dirty = new int[0x1000];

            memset(HuC6270_vram, 0, 0x20000);
            memset(u8_tile_dirty, 1, 0x1000);
            memset(u8_sprite_dirty, 1, 0x400);
            memset(u8_vram_dirty, 1, 0x1000);

            tile_bitmap = bitmap_alloc(512, 512);
            front_bitmap = bitmap_alloc(512, 512);

            if (tile_bitmap == null || front_bitmap == null || u8_tile_dirty == null || HuC6270_vram == null || u8_sprite_dirty == null || u8_vram_dirty == null) {
                return 1;
            }
