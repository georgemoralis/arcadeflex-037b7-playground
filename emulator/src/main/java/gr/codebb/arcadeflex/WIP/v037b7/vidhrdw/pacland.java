...
public class pacland
{
	...
	***************************************************************************/
	...
	public static void COLOR(char []colortable, int gfxn, int offs, int value){ colortable[Machine.drv.gfxdecodeinfo[gfxn].color_codes_start + offs] = (char) value; }
        public static int COLOR(char []colortable, int gfxn, int offs){ return colortable[Machine.drv.gfxdecodeinfo[gfxn].color_codes_start + offs]; }
	...
	public static VhConvertColorPromPtr pacland_vh_convert_color_prom = new VhConvertColorPromPtr() { public void handler(char []palette, char []colortable, UBytePtr color_prom) 
	{
		...
		for (i = 0;i < TOTAL_COLORS(2)/3;i++)
		{
			...
			if (COLOR(colortable,2,i) == 0x7f) COLOR(colortable,2,i + 2*TOTAL_COLORS(2)/3, COLOR(colortable,2,i));
			else COLOR(colortable,2,i + 2*TOTAL_COLORS(2)/3, 0xff);
	
			/* transparent colors are 0x7f and 0xff - map all to 0xff */
			if (COLOR(colortable,2,i) == 0x7f) COLOR(colortable,2,i, 0xff);
	
			/* high priority colors which appear over the foreground even when */
			/* the foreground has priority over sprites */
			if (COLOR(colortable,2,i) >= 0xf0) COLOR(colortable,2,i + TOTAL_COLORS(2)/3, COLOR(colortable,2,i));
			else COLOR(colortable,2,i + TOTAL_COLORS(2)/3, 0xff);
		}
		...
		for (i = 0;i < TOTAL_COLORS(0);i++)
		{
			...
			/* transparent colors are 0x7f and 0xff - map all to 0xff */
			if (COLOR(colortable,0,i) == 0x7f) COLOR(colortable,0,i, 0xff);
		}
