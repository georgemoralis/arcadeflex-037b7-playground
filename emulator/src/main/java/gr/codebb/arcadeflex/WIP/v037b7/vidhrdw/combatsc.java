...
public class combatsc {
    ...
    static UBytePtr combasc_io_ram;
    static int combasc_vreg;
    public static UBytePtr banked_area = new UBytePtr();

    static int combasc_bank_select;
    /* 0x00..0x1f */
    static int combasc_video_circuit;
    /* 0 or 1 */
    static UBytePtr combasc_page[] = new UBytePtr[2];
    static /*unsigned*/ char[] combasc_scrollram0 = new char[0x40];
    static /*unsigned*/ char[] combasc_scrollram1 = new char[0x40];
            ...
            for (pal = 0; pal < 8; pal++) {
                switch (pal) {
                    ...
                    case 2:
                        /* other sprites(alt) */
                        clut = 1;
                        /* 0 is wrong for Firing Range III targets */
                        break;
    ...
    public static VhConvertColorPromPtr combascb_convert_color_prom = new VhConvertColorPromPtr() {
        public void handler(char[] palette, char[] colortable, UBytePtr color_prom) {
            ...
            for (pal = 0; pal < 8; pal++) {
                for (i = 0; i < 256; i++) {
                    if ((pal & 1) == 0) /* sprites */ {
                        colortable[c_ptr++] = (char) (16 * pal + (color_prom.read(i) ^ 0x0f));
                    } else /* chars */ {
                        colortable[c_ptr++] = (char) (16 * pal + (i & 0x0f));	/* no lookup? */
                    }
                }
            }
        }
    ...
    public static GetTileInfoPtr get_tile_info0 = new GetTileInfoPtr() {
        public void handler(int tile_index) {
            /*unsigned*/ char attributes = combasc_page[0].read(tile_index);
            ...
            tile_info.u32_priority = (attributes & 0x40) >> 6;
        }
    };

    public static GetTileInfoPtr get_tile_info1 = new GetTileInfoPtr() {
        public void handler(int tile_index) {
            /*unsigned*/ char attributes = combasc_page[1].read(tile_index);
    ...
    public static GetTileInfoPtr get_text_info = new GetTileInfoPtr() {
        public void handler(int tile_index) {
            /*unsigned*/ char attributes = combasc_page[0].read(tile_index + 0x800);
    ...
    public static GetTileInfoPtr get_tile_info0_bootleg = new GetTileInfoPtr() {
        public void handler(int tile_index) {
            /*unsigned*/ char attributes = combasc_page[0].read(tile_index);
            int bank = 4 * ((combasc_vreg & 0x0f) - 1);
    ...
    public static GetTileInfoPtr get_tile_info1_bootleg = new GetTileInfoPtr() {
        public void handler(int tile_index) {
            /*unsigned*/ char attributes = combasc_page[1].read(tile_index);
            int bank = 4 * ((combasc_vreg >> 4) - 1);
            int number, pal, color;
    ...
    public static GetTileInfoPtr get_text_info_bootleg = new GetTileInfoPtr() {
        public void handler(int tile_index) {
     ...
     **************************************************************************
     */
    public static VhStartPtr combasc_vh_start = new VhStartPtr() {
        public int handler() {
            combasc_vreg = -1;
    ...
    public static VhStartPtr combascb_vh_start = new VhStartPtr() {
        public int handler() {
            combasc_vreg = -1;
    ...
    public static VhStopPtr combasc_vh_stop = new VhStopPtr() {
        public void handler() {
     ...
     **************************************************************************
     */
    public static ReadHandlerPtr combasc_video_r = new ReadHandlerPtr() {
        public int handler(int offset) {
            return videoram.read(offset);
        }
    };

    public static WriteHandlerPtr combasc_video_w = new WriteHandlerPtr() {
    ...
    public static WriteHandlerPtr combasc_vreg_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
    ...
    public static WriteHandlerPtr combascb_sh_irqtrigger_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
    ...
    public static ReadHandlerPtr combasc_io_r = new ReadHandlerPtr() {
        public int handler(int offset) {
    ...
    public static WriteHandlerPtr combasc_io_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
            switch (offset) {
                case 0x400:
                    priority = data & 0x20;
                    break;
                case 0x800:
                    combascb_sh_irqtrigger_w.handler(0, data);
                    break;
                case 0xc00:
                    combasc_vreg_w.handler(0, data);
                    break;
                default:
    ...
    public static WriteHandlerPtr combasc_bankselect_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
    ...
    public static WriteHandlerPtr combascb_bankselect_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
            if ((data & 0x40) != 0) {
                combasc_video_circuit = 1;
                videoram = combasc_page[1];
    ...
    public static InitMachinePtr combasc_init_machine = new InitMachinePtr() {
        public void handler() {
    ...
    public static WriteHandlerPtr combasc_pf_control_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
            ...
            if (offset == 3) {
                if ((data & 0x08) != 0) {
                    memcpy(private_spriteram[combasc_video_circuit], new UBytePtr(combasc_page[combasc_video_circuit] , 0x1000), 0x800);
                } else {
                    memcpy(private_spriteram[combasc_video_circuit], new UBytePtr(combasc_page[combasc_video_circuit] , 0x1800), 0x800);
                }
            }
        }
    };

    public static ReadHandlerPtr combasc_scrollram_r = new ReadHandlerPtr() {
        public int handler(int offset) {
            return combasc_scrollram.read(offset);
        }
    };

    public static WriteHandlerPtr combasc_scrollram_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
     ...
     **************************************************************************
     */
    static void draw_sprites(osd_bitmap bitmap, UBytePtr source, int circuit,/*UINT32*/ int pri_mask) {
    ...
    public static VhUpdatePtr combasc_vh_screenrefresh = new VhUpdatePtr() {
        public void handler(osd_bitmap bitmap, int full_refresh) {
            ...
            if ((K007121_ctrlram[0][0x01] & 0x08)!=0) {
                for (i = 0; i < 32; i++) {
                    tilemap_set_scrollx(textlayer, i, combasc_scrollram0[0x20 + i] != 0 ? 0 : TILE_LINE_DISABLED);
     ...
     *
     * byte #0:	sprite number byte #1:	y position byte #2:	x position byte #3:
     * bit 0:	x position (bit 0) bits 1..3:	??? bit 4:	flip x bit 5:	unused? bit
     * 6:	sprite bank # (bit 2) bit 7:	??? byte #4: bits 0,1:	sprite bank #
     * (bits 0 & 1) bits 2,3:	unused? bits 4..7:	sprite color
     *
     **************************************************************************
     */
    static void bootleg_draw_sprites(osd_bitmap bitmap, UBytePtr source, int circuit) {
        ...
        UBytePtr finish = new UBytePtr();

        source.offset += 0x1000;
        finish = new UBytePtr(source);
        source.offset += 0x400;
        limit = (0x3400 - limit) / 8;
        if (limit >= 0) {
            finish.offset = source.offset - limit * 8;
        }
        source.offset -= 8;

        while (source.offset > finish.offset) {
            int/*unsigned char*/ attributes = source.read(3);
            /* PBxF ?xxX */
            {
    ...
    public static VhUpdatePtr combascb_vh_screenrefresh = new VhUpdatePtr() {
        public void handler(osd_bitmap bitmap, int full_refresh) {
            int i;
