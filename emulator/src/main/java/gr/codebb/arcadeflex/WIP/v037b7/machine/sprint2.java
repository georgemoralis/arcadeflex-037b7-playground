...
public class sprint2
{
	...
	public static int sprint2_collision1_data = 0;
	public static int sprint2_collision2_data = 0;
	public static int sprint2_gear1 = 1;
	public static int sprint2_gear2 = 1;
	
	public static int sprint2_steering_buf1 = 0;
	public static int sprint2_steering_buf2 = 0;
	public static int sprint2_steering_val1 = 0xFF;
	public static int sprint2_steering_val2 = 0xFF;
	...
	***************************************************************************/
	...
	public static ReadHandlerPtr sprint1_read_ports_r  = new ReadHandlerPtr() { public int handler(int offset)
	{
		...
		gear=input_port_1_r.handler(0);
		if ((gear & 0x01) != 0)                sprint2_gear1=1;
		else if ((gear & 0x02) != 0)           sprint2_gear1=2;
		else if ((gear & 0x04) != 0)           sprint2_gear1=3;
		else if ((gear & 0x08) != 0)           sprint2_gear1=4;
		...
		switch (offset)
		{
		        ...
		        case 0x2B:      return ((input_port_2_r.handler(0) & 0x01) << 7);
		        case 0x2C:      return ((input_port_2_r.handler(0) & 0x02) << 6);
		        case 0x2D:      return ((input_port_2_r.handler(0) & 0x04) << 5);
		        /* DSW */
		        case 0x10:
		        case 0x14:
		        case 0x30:
	...
	***************************************************************************/
	...
	public static ReadHandlerPtr sprint2_read_ports_r  = new ReadHandlerPtr() { public int handler(int offset)
	{
		...
		if ((gear & 0x10) != 0)                sprint2_gear2=1;
		else if ((gear & 0x20) != 0)           sprint2_gear2=2;
		else if ((gear & 0x40) != 0)           sprint2_gear2=3;
		else if ((gear & 0x80) != 0)           sprint2_gear2=4;
		...
		switch (offset)
		{
		        ...
		        case 0x2C:      return ((input_port_2_r.handler(0) & 0x08) << 4);
		        case 0x2D:      return ((input_port_2_r.handler(0) & 0x10) << 3);
		        case 0x2E:      return ((input_port_2_r.handler(0) & 0x20) << 2);
		        /* IN1 */
	...
	***************************************************************************/
	...
	public static ReadHandlerPtr sprint2_read_sync_r  = new ReadHandlerPtr() { public int handler(int offset)
	{
		ac_line=(ac_line+1) % 3;
		if (ac_line==0)
		        return ((input_port_3_r.handler(0) & 0x7f) | 0x80);
		else
		        return (input_port_3_r.handler(0) & 0x7F);
	} };
	...
	***************************************************************************/
	...
	public static ReadHandlerPtr sprint2_steering2_r  = new ReadHandlerPtr() { public int handler(int offset)
	{
		...
		this_val=input_port_6_r.handler(0);
		delta=this_val-last_val_2;
		last_val_2=this_val;
		if (delta>128) delta-=256;
		else if (delta<-128) delta+=256;
		/* Divide by four to make our steering less sensitive */
